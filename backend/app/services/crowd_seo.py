...
# Runtime code can execute from an immutable release tree. Mutable Crowd SEO
# state must always live under the canonical writable backend state root.
BACKEND_ROOT = Path(os.environ.get("BORIS_STATE_BACKEND_ROOT") or Path(__file__).resolve().parents[2])
ROOT = BACKEND_ROOT / "data" / "crowd_seo"
PROJECTS_FILE = ROOT / "projects.json"
PROJECTS_LOCK_FILE = ROOT / "projects.lock"
GUEST_CONFIRMED_DIR = BACKEND_ROOT / "data" / "service_marketplaces" / "guest_publish_runs"
...
def _legacy_offer_type(project: dict) -> str:
    ...
    scope = str(profile.get("campaign_scope") or project.get("campaign_scope") or "").lower()
    policy = str(profile.get("content_policy") or project.get("content_policy") or "").lower()
    purpose = str(profile.get("purpose") or "").lower()
    # A development-only campaign sells professional services even when the
    # landing page mentions client products, shops or goods in examples. The
    # old persisted `mixed` value let goods-only boards leak into the live plan.
    if (
        "development_only" in scope
        or "development_only" in policy
        or purpose == "lead_generation_for_custom_software_development"
    ):
        return "services"
        ...
        for draft in marketplace.list_drafts():
            if (
                ...
                try:
                    ...
                    observe_suppressed(__name__, _suppressed_exc, line=461)
...
def _fetch(url: str) -> tuple[str, str]:
    ...
    if _html_needs_browser_render(r.text):
        try:
            ...
            observe_suppressed(__name__, _suppressed_exc, line=518)
    return r.url, r.text
...
def _confirmed_guest_evidence(project: dict) -> list[dict]:
    """Return verified guest publications that belong to this project's site.

    Guest/no-registration publishers predate the Crowd SEO placement ledger.
    Keep their immutable evidence as a separate source and reconcile it at read
    time so a proven public post is counted without inventing a forum slot or
    risking a duplicate publication.
    """
    site_host=urlparse(str(project.get("site") or "")).netloc.lower().removeprefix("www.")
    if not site_host or not GUEST_CONFIRMED_DIR.exists():
        return []
    canonical_publications = {
        str(row.get("url") or "").rstrip("/"): row
        for row in marketplace.list_publications()
        if str(row.get("url") or "").strip()
    }
    out=[]; seen=set()
    for path in sorted(GUEST_CONFIRMED_DIR.glob("confirmed_posts_*.json")):
        try:
        ...
        for row in rows:
            ...
            publication=str(row.get("publication_url") or "").strip()
            if urlparse(target).netloc.lower().removeprefix("www.") != site_host or not publication:
                continue
            key=publication.rstrip("/")
            if key in seen:
                continue
            current = canonical_publications.get(key)
            # Historical sidecar evidence proves that a post once existed, but
            # current Crowd SEO credit must follow the latest live verifier.
            if current is not None and (
                current.get("verified") is False
                or bool(current.get("verification_error"))
                or current.get("link_present") is False
            ):
                continue
...
def _placement_summary(project: dict) -> dict:
    ...
    guest_verified=_confirmed_guest_evidence(project)
    placement_urls={str(x.get("publication_url") or "").rstrip("/") for x in published}
    guest_verified=[x for x in guest_verified if str(x.get("publication_url") or "").rstrip("/") not in placement_urls]
    contract_covered=len(active)+len(guarantee_expired)
    return {
        "planned":planned,
        "active":len(active),
        "history":len(history),
        "published":len(published)+len(guest_verified),
        "verified":len(verified)+len(guest_verified),
        "placement_published":len(published),
        "placement_verified":len(verified),
        "guest_verified":len(guest_verified),
        "guarantee_expired":len(guarantee_expired),
        "contract_covered":contract_covered,
        "active_deficit":max(0,planned-contract_covered),
    }

def _effective_target_pages(project: dict) -> list[dict]:
    """Return the landing pages that may be used by new placements.

    Software-development campaigns must route every post to a concrete
    /software-dev/services/... landing. The campaign's primary_url is only the
    campaign root and must never override the per-service post targets.
    """
    site = str(project.get("site") or "")
    profile = project.get("campaign_profile") or {}
    posts = list(profile.get("posts_ru") or [])
    if "/software-dev" in site and posts:
        out=[]; seen=set()
        for post in posts:
            url=str(post.get("target_url") or "").strip()
            if "/software-dev/services/" not in url or url in seen:
                continue
            seen.add(url)
            out.append({"url":url,"label":str(post.get("title") or project.get("niche_label") or "Разработка ПО")})
        if out:
            return out
    primary=str(profile.get("primary_url") or "").strip()
    if primary:
        return [{"url":primary,"label":str(profile.get("brand") or project.get("niche_label") or "сайт")}]
    return list(project.get("target_pages") or [{"url":project.get("site"),"label":project.get("niche_label","сайт")}])
...
def _retarget_unpublished_placements(project: dict) -> int:
    ...
    """
    targets=_effective_target_pages(project)
...
def _usable_matches_for_project(project: dict, matches: list[dict]) -> list[dict]:
    """Exclude surfaces already consumed by a published post that failed verification."""
    consumed = {
        _slot_key(pl)
        for pl in (project.get("placements") or [])
        if pl.get("publication_url")
        and str(pl.get("status") or "") in {"replacement_required", "platform_replacement_required"}
    }
    if not consumed:
        return list(matches)
    return [row for row in matches if _slot_key(row) not in consumed]
...
def _forum_matches(
    ...
    for p in marketplace.list_platforms():
        channel_type = str(p.get("channel_type") or "")
        explicit_surfaces = list(p.get("publication_surfaces") or [])
        # Existing engine also handles verified own-listing catalog surfaces.
        # Generic communities remain excluded without an explicit surface.
        if channel_type != "forum" and not explicit_surfaces:
            continue
...
def bootstrap_priority_snapshot(projects: list[dict] | None = None) -> dict[str, dict]:
    ...
    for project in active_projects:
        if not project_is_active(project):
            continue
        # Inventory projects are not client-urgent, but they still provide a
        # soft relevance signal so the reusable worker prepares surfaces that
        # can unlock the currently active BORIS campaign before unrelated
        # reserve. Only real client contracts may hard-scope the queue.
        ...
        for platform, surfaces in per_platform_surfaces.items():
            ...
            row["urgent_for_active_projects"] = bool(
                row.get("urgent_for_active_projects") or not is_inventory
            )
    ...
    return {
        platform: {
            **row,
            ...
            "urgent_for_active_projects": bool(row.get("urgent_for_active_projects")),
        }
...
def _match_reachable_after_bootstrap(match: dict) -> bool:
    return bool(
        match.get("publication_ready")
        or (
            match.get("account_status") == "in_progress"
            and match.get("checkpoint") == "form_prepared"
        )
        or (
            str(match.get("checkpoint") or "") in marketplace.HUMAN_PLATFORM_BOOTSTRAP_CHECKPOINTS
            and not match.get("maturity_required")
        )
    )


def _match_reachable_after_warmup(match: dict) -> bool:
    return bool(
        _match_reachable_after_bootstrap(match)
        or match.get("account_warming")
        or match.get("maturity_required")
        or str(match.get("checkpoint") or "") in marketplace.WARMING_REGISTRATION_CHECKPOINTS
    )


def _match_activation_tier(match: dict) -> int:
    """Stable selection priority: READY -> bootstrap -> warmup -> unreachable."""
    if match.get("publication_ready"):
        return 0
    if _match_reachable_after_bootstrap(match):
        return 1
    if _match_reachable_after_warmup(match):
        return 2
    return 3


def _capacity_for_project(project: dict, matches: list[dict] | None = None) -> dict:
    contract_required=_project_required_count(project)
    if matches is None:
        matches=_forum_matches(
            project.get("niche") or "business",
            limit=max(100,contract_required*5),
            keywords=_project_keywords(project),
            offer_type=_legacy_offer_type(project),
        )
    matches=_usable_matches_for_project(project, list(matches))
    rule_verified=[
        x for x in matches
        if (
    ...
    reachable_after_bootstrap=[
        x for x in rule_verified
        if _match_reachable_after_bootstrap(x)
    ]
    reachable_after_warmup=[
        x for x in rule_verified
        if _match_reachable_after_warmup(x)
    ]
    # INVENTORY_CAPACITY_MOVING_TARGET_FIX_V1
    # A reusable inventory campaign has no fixed client quantity. Treating
    # every rule-allowed candidate as `required` creates an impossible moving
    # target: each newly discovered replacement increases both capacity and the
    # requirement by one, so a single transiently unreachable site can keep a
    # permanent discovery deficit forever. Keep such sites in the retry pool,
    # but define current operational demand by the surfaces that are actually
    # reachable after their legitimate one-time bootstrap/warmup path.
    #
    # Contract projects remain strict: their explicit paid quantity is never
    # weakened by `publish_all_eligible` or by a temporarily unavailable site.
    is_inventory = bool(
        str(project.get("plan_mode") or "") == "inventory"
        or (project.get("publish_all_eligible") and contract_required <= 0)
    )
    operational_required = len(reachable_after_warmup) if is_inventory else contract_required
    inventory_deferred_unreachable = max(0, len(rule_verified) - len(reachable_after_warmup)) if is_inventory else 0
    activation_deficit_after_bootstrap = max(0, operational_required - len(reachable_after_bootstrap))
    activation_deficit_after_warmup = max(0, operational_required - len(reachable_after_warmup))
    discovery_deficit_after_bootstrap = (
        0 if is_inventory else max(0, contract_required - len(reachable_after_bootstrap))
    )
    discovery_deficit_after_warmup = (
        0 if is_inventory else max(0, contract_required - len(reachable_after_warmup))
    )
    return {
        "required":operational_required,
        "contract_required":contract_required,
        "operational_required":operational_required,
        "inventory_mode":is_inventory,
        "inventory_rule_verified_total":len(rule_verified) if is_inventory else 0,
        "inventory_deferred_unreachable":inventory_deferred_unreachable,
        ...
        "autonomous_deficit":max(0,operational_required-len(autonomous)),
        "ready_deficit":max(0,operational_required-len(ready)),
        "activation_deficit_after_bootstrap":activation_deficit_after_bootstrap,
        "activation_deficit_after_warmup":activation_deficit_after_warmup,
        "discovery_deficit_after_bootstrap":discovery_deficit_after_bootstrap,
...
def capacity_snapshot() -> list[dict]:
    ...
    for project in list_projects():
        ...
        live_slots={_slot_key(c) for c in (project.get("forum_candidates") or [])}
        covered_live_slots=sum(
            1 for placement in (project.get("placements") or [])
            if _slot_key(placement) in live_slots
            and (
                _placement_is_active(placement)
                or str(placement.get("status") or "") == "guarantee_expired"
            )
        )
        placement_operational_deficit=max(0,int(cap.get("required") or 0)-covered_live_slots)
        out.append({
            "project":project.get("id"),"site":project.get("site"),"niche":project.get("niche") or "business",
            **cap,
            "placement_covered_live_slots":covered_live_slots,
...
def prepare_placement_plan(project_id: str) -> dict:
    ...
    for i,forum in enumerate(forums[:wanted]):
        ...
        placement={
            ...
            "anchor":a["anchor"],"text_variant":variant,"text":"","status":state,
        ...
        # PLACEMENT_TEXT_EXACT_TARGET_V1: project state and publish draft must
        # be rendered from the same exact target URL. Never persist a generic
        # root-site text next to a software-development service target.
        _, placement["text"] = _render_placement_text(project, placement, variant)
...
# CROWD_SEO_BROWSER_VERIFY_FALLBACK_V1
# Ordinary HTTP probes can be blocked by a forum anti-bot layer even when the
# already-published topic is healthy in BORIS' persisted browser session.  This
# fallback is read-only: it never clicks publish or bypasses a checkpoint.  It
# only asks the existing marketplace browser runner to prove that the exact
# target link is present on the exact saved publication URL.
def _browser_verify_publication(platform: str, publication_url: str, target_url: str) -> dict:
    script = BACKEND_ROOT / "tools" / "marketplace_browser_assistant.py"
    if not platform or not publication_url or not target_url or not script.exists():
        return {"ok": False, "publication_verified": False, "error": "browser_verify_preflight_failed"}
    try:
        proc = subprocess.run(
            [
                sys.executable,
        ...
        return {"ok": False, "publication_verified": False, "error": "browser_verify_timeout"}
    ...
    if proc.returncode != 0 or not isinstance(payload, dict):
        return {
            ...
            "error": ((proc.stderr or proc.stdout or "browser_verify_failed")[-1000:]).strip(),
        }
    return payload
...
def verify_placement(project_id: str, placement_id: str) -> dict:
    ...
    try:
        ...
        for a in soup.find_all("a",href=True):
            ...
            # CROWD_PLACEMENT_EXACT_HREF_V1: link text is not backlink proof.
            # Credit only an href that resolves to the exact target URL.
            if (
        ...
        if pl["link_present"] and pl["indexable"]:
            pl["status"]="verified"
            pl["verification_transport"]="http"
            pl["error"]=None
        elif r.status_code in {401,403,429} or r.status_code >= 500:
            # BROWSER_VERIFY_ON_HTTP_BLOCK_V1: a forum may deny the plain HTTP
            # verifier while the already-created topic is healthy in the saved
            # BORIS browser session. Prove the exact target link there before
            # leaving the placement in a transient state. No clicks/writes.
            browser = _browser_verify_publication(
                str(pl.get("platform") or ""),
                str(pub),
                str(target),
            )
            if browser.get("publication_verified") is True:
                pl["status"]="verified"
                pl["verification_transport"]="browser_session"
                pl["http_status"]=int(browser.get("http_status") or 200)
                pl["final_publication_url"]=str(browser.get("final_url") or pub)
                pl["link_present"]=True
                pl["indexable"]=bool(browser.get("indexable", True))
                pl["rel"]=browser.get("rel")
                pl["link_rel_type"]=browser.get("link_rel_type") or (
                    "nofollow" if "nofollow" in str(browser.get("rel") or "").lower() else "dofollow"
                )
                pl["error"]=None
            elif browser.get("ok") is True and int(browser.get("http_status") or 0) == 200:
                # The persisted browser session successfully loaded the exact
                # publication page, but the target link is absent or the page is
                # noindex. That is conclusive negative proof, not a transport
                # failure: stop counting the old link and schedule replacement.
                pl["status"]="replacement_required"
                pl["verification_transport"]="browser_session"
                pl["http_status"]=200
                pl["final_publication_url"]=str(browser.get("final_url") or pub)
                pl["link_present"]=bool(browser.get("link_present"))
                pl["indexable"]=bool(browser.get("indexable"))
                pl["rel"]=browser.get("rel")
                pl["link_rel_type"]=browser.get("link_rel_type")
                pl["error"]=str(browser.get("error") or "target_link_missing_or_not_indexable")[:1000]
            else:
                # A verifier-side access/rate/server failure is not evidence that
                # a previously verified publication disappeared. Keep the last
                # public proof and retry later instead of creating a false
                # replacement/duplicate publication.
                pl["status"]="verification_temporarily_blocked"
                pl["verification_transport"]="http_blocked_browser_unconfirmed"
                pl["error"]=str(browser.get("error") or f"verification_http_{int(r.status_code)}")[:1000]
        else:
            pl["status"]="replacement_required"
            pl["verification_transport"]="http"
            pl["error"]="link_missing_or_not_indexable"
        ...
        if browser.get("publication_verified") is True:
            ...
            pl["status"]="verification_temporarily_blocked"
            pl["verification_transport"]="http_error_browser_unconfirmed"
            pl["error"]=str(browser.get("error") or f"{type(exc).__name__}: {exc}")[:1000]
    project["updated_at"]=_now()
    _save(rows)
    return pl
...
def _render_placement_text(project: dict, pl: dict, variant: int) -> tuple[str,str]:
    ...
    if profile_posts:
        ...
        placement_target=str(pl.get("target_url") or "").strip().rstrip("/")
        # PLACEMENT_TARGET_PROFILE_BINDING_V1:
        # The exact placement landing is authoritative. Replacement/top-up
        # slots can inherit a stale keyword/variant, so keyword scoring alone
        # may otherwise select SaaS copy for an IP-telephony landing. Restrict
        # profile routing to posts for the exact target whenever such posts
        # exist; only legacy targets without a mapped post use the old pool.
        exact_target_posts=[
            candidate for candidate in profile_posts
            if placement_target
            and str(candidate.get("target_url") or "").strip().rstrip("/") == placement_target
        ]
        routing_posts=exact_target_posts or profile_posts
        for candidate in routing_posts:
            ...
            # Explicit product cluster is authoritative only inside the exact
            # target pool. Text matching remains a legacy compatibility hint.
        ...
        if post is None:
            post=routing_posts[(max(1,int(variant))-1) % len(routing_posts)]
        brand=str(profile.get("brand") or project.get("domain") or "BORIS")
        # The placement owns the destination. Exact-target routing above keeps
        # the selected title/body in the same intent family; a profile target
        # remains only a fallback for legacy placements without target_url.
        url=str(pl.get("target_url") or post.get("target_url") or project.get("site") or "")
        title=str(post.get("title") or brand).replace("{brand}",brand).replace("{url}",url)
        text=str(post.get("body") or post.get("text") or "").replace("{brand}",brand).replace("{url}",url)
        if url and url.lower() not in text.lower():
            link_label="More details" if language=="en" else ("Подробнее о продукте" if str(post.get("offer_family") or "") == "product" else "Подробнее об услуге")
            text=f"{text}\n\n{link_label}: {url}"
        # CONTENT_GATE_FOOTER_PRESERVATION_V1:
        # `_clean_text` is intentionally for scraped/page text and collapses
        # all whitespace. Applying it after building the publication copy also
        # collapsed the required final keyword paragraph, so every otherwise
        # valid development draft failed `keyword_block_not_last`. Clean the
        # human-readable body first, then append one preserved final paragraph
        # in exactly the same format enforced by validate_publication_content().
        text=_clean_text(text)
        keywords=[str(x).strip() for x in (post.get("keywords") or []) if str(x).strip()]
        exact_keyword=str(pl.get("keyword") or "").strip()
        if exact_keyword and exact_keyword.lower() not in {x.lower() for x in keywords}:
            keywords.insert(0, exact_keyword)
        if keywords:
            label="Keyword phrases" if language=="en" else "Ключевые фразы"
            text=f"{text}\n\n{label}: " + "; ".join(keywords[:20]) + "."
        return _clean_text(title)[:180], text[:1800]
...
def schedule_replacements(project_id: str) -> dict:
    ...
    live_matches=_usable_matches_for_project(
        project,
        _forum_matches(
            project.get("niche") or "business",
            limit=80,
    ...
    for old in list(project.get("placements",[])):
        ...
        state=("ready_to_publish" if project.get("content_status")=="approved" else "ready_for_content_approval") if forum.get("publication_ready") else ("external_checkpoint" if forum.get("account_status") in {"verification_required","in_progress"} else "waiting_platform_ready")
        repl={
            ...
            "text_variant":old.get("text_variant",1),"text":"","status":state,
            "publication_url":None,"checked_at":None,"link_present":None,"indexable":None,"rel":None,
            "http_status":None,"final_publication_url":None,
            "relevance":forum.get("relevance"),"rule_status":forum.get("rule_status"),
            "surface_evidence":forum.get("surface_evidence"),
        ...
        _, repl["text"] = _render_placement_text(
            project, repl, int(repl.get("text_variant") or 1)
        )
    ...
    # All currently eligible slots already represented by a placement count
    # toward the inventory plan, including published history. Failed historical
    # slots do not. This avoids both false deficits and false saturation.
    live_slots={_slot_key(c) for c in (project.get("forum_candidates") or [])}
    active_count=sum(
        1 for x in project.get("placements",[])
        if _slot_key(x) in live_slots
        and (_placement_is_active(x) or bool(x.get("publication_url")))
    )
    profile=project.get("campaign_profile") or {}
    ...
    while active_count < wanted and candidates:
        ...
        state=("ready_to_publish" if project.get("content_status")=="approved" else "ready_for_content_approval") if forum.get("publication_ready") else (
            "external_checkpoint"
        ...
        _, placement["text"] = _render_placement_text(
            project, placement, int(placement.get("text_variant") or 1)
        )
...
def refresh_project_forums(project_id: str) -> dict:
    ...
    if not project:
        raise KeyError(project_id)
    fresh=_usable_matches_for_project(
        project,
        _forum_matches(
            project.get("niche") or "business",
            limit=80,
    ...
    changed=0
    for pl in project.get("placements",[]):
        if pl.get("publication_url") or pl.get("status") in {"verified","replacement_required","published_unverified","publication_pending_verification","verification_temporarily_blocked"}:
            # Published history is immutable during candidate refresh. A
            # transient verifier block (403/429/5xx) must never make an old URL
            # look READY again and trigger a duplicate publish attempt.
            continue
