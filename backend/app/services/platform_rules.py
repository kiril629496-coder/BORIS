...
PAID_ONLY = [
    ...
    r"рекламн(?:ая|ое|ый|ые).{0,120}\bплатн",
    r"размещение.{0,120}\bплатн",
    r"реклама.{0,120}через администратор",
    r"по вопросам рекламы.{0,120}администратор",
    r"стоимость размещения",
    r"\bплатн.{0,60}услуг.{0,220}меньше\s+100\s+сообщен.{0,260}(?:1000|тысяч).{0,60}руб",
...
def _inspect_platform_legacy_v1(platform_key: str) -> dict:
    ...
    # PUBLICATION_DISCOVERY_FAIL_CLOSED_V1:
    # A search hit can be an article *about* advertising/blogs. Positive words
    # there do not prove that this domain itself accepts our own listing/post.
    # Require a concrete same-domain authoring route before enabling outreach.
    if platform_key.startswith("pubdisc_"):
        try:
            from app.services import publication_discovery
            candidate = next(
                (
                    x for x in (publication_discovery.load() or {}).get("items", [])
                    if f"pubdisc_{re.sub(r'[^a-z0-9]+', '_', str(x.get('domain') or '').lower()).strip('_')[:48]}"
                    == platform_key
                ),
                None,
            )
        ...
        author_markers = (
            ...
            "post-ad", "post_ad", "new-ad", "new_ad", "create-listing",
            "create_listing", "add-company", "add_company", "submit-company",
            "submit_company", "new-post", "new_post", "/write", "/publish", "/new/",
        )
        same_domain_authoring = False
        if candidate:
            ...
            for item in inspections:
                for link in (item.get("outbound_links") or []):
                    try:
                        ...
                        continue
                    if host == domain and any(marker in route for marker in author_markers):
                        same_domain_authoring = True
                        break
                if same_domain_authoring:
                    break
        if not same_domain_authoring:
            final = "review"
            requirements = sorted(set(requirements) | {"exact_same_domain_authoring_surface_required"})

    # PUBLICATION_DISCOVERY_FAIL_CLOSED_V1:
    # Search can return an article about advertising/blogging. That is not
    # evidence that the same domain accepts our own listing or article.
    if platform_key.startswith("pubdisc_"):
        try:
            from app.services import publication_discovery
            candidate = next(
    ...
    # ORGPAGE_CURATED_FREE_COMPANY_CARD_V1
    # Orgpage is allowed only with three independent pieces of current official
    # evidence: the exact create route, explicit zero-cost baseline placement,
    # and an official profile field for the company's external website.
    if platform_key == "pubdisc_orgpage_ru":
        authoring = _publication_discovery_authoring_evidence(platform_key, inspections)
        create_text = " ".join(
            str(x.get("text_excerpt") or "").lower()
            for x in inspections
            if "orgpage.ru/cabinet/create" in str(x.get("url") or "").lower()
        )
        help_text = " ".join(
            str(x.get("text_excerpt") or "").lower()
            for x in inspections
            if "orgpage.usedocs.com/article/64533" in str(x.get("url") or "").lower()
        )
        free_baseline = (
            "добавьте вашу компанию в каталог" in create_text
            and "абсолютно бесплатно" in create_text
        )
        website_supported = (
            "добавить ссылку на сайт компании" in help_text
            and "поле «сайт»" in help_text
        )
        prohibited = any(
            bool((x.get("evidence") or {}).get("prohibited"))
            or bool((x.get("evidence") or {}).get("links"))
            for x in inspections
        )
        if authoring.get("found") and free_baseline and website_supported and not prohibited:
            final = "allowed"
            requirements = sorted(set(requirements) | {
                "use_own_company_card_only",
                "free_company_listing_verified",
                "website_link_supported_in_company_profile",
    ...
    # Yell's free baseline is documented, but until the free company-card path
    # also proves an external website field, keep it fail-closed for Crowd SEO.
    # GDE_FREE_LISTING_MANDATORY_LINK_CONFLICT_V1
    # The free form explicitly makes the external website link paid and forbids
    # contact details in title/body, which conflicts with mandatory BORIS link.
    if platform_key == "pubdisc_gde_ru":
        gde_text = " ".join(str(x.get("text_excerpt") or "").lower() for x in inspections)
        if (
            "ссылка на сайт (платно)" in gde_text
            and "не указывайте контактные данные в заголовке и тексте объявления" in gde_text
        ):
            final = "blocked"
            requirements = sorted(set(requirements) | {
                "external_website_link_is_paid",
                "contact_details_forbidden_in_title_and_body",
                "mandatory_boris_link_incompatible_with_free_tier",
            })
        else:
            final = "review"

    # SAASHUB_SOFTWARE_AGENCY_INCOMPATIBLE_V1
    # SaaSHub's own submit guidelines explicitly reject software-development
    # agencies. This Crowd SEO project sells custom software development, so
    # keeping SaaSHub in bootstrap would only consume registration/audit cycles
    # and risk a knowingly invalid submission.
    if platform_key == "pubdisc_saashub_com":
        saashub_text = " ".join(str(x.get("text_excerpt") or "").lower() for x in inspections)
        if "software development agencies" in saashub_text and "not accepted" in saashub_text:
            final = "blocked"
            requirements = sorted(set(requirements) | {
                "software_development_agencies_not_accepted",
                "custom_development_offer_incompatible_with_product_directory",
            })
        else:
            final = "review"

    if platform_key == "pubdisc_yell_ru":
        final = "review"
        requirements = sorted(set(requirements) | {
            "free_external_website_link_not_yet_verified",
        })
...
RULE_URLS.update({
    ...
    "joomlaforum_jobs": [
        "https://joomlaforum.ru/index.php/board,29.0.html",
        "https://joomlaforum.ru/index.php/topic,17.0.html",
    ],
})
...
def inspect_url_browser(url: str, *, platform_key: str | None = None) -> dict:
    """Read public rule text with a browser when normal HTTP is blocked/empty.

    For a platform where BORIS already owns a legitimate persisted browser
    profile, reuse that session read-only instead of opening a fresh anonymous
    Chromium profile that can be blocked by the site's anti-bot layer. This
    never clicks, submits, accepts terms or solves a challenge.
    """
    from playwright.sync_api import sync_playwright
    persistent_dir = None
    if platform_key:
        candidate = ROOT / "browser_sessions" / "service_marketplaces" / str(platform_key)
    ...
    with sync_playwright() as pw:
        ...
        try:
            if persistent_dir is not None:
                ...
                transport = "persisted_browser_session"
            else:
...
# Curated strong catalogs still go through the normal rule engine. Audit the
# exact create route plus official help proving that a website link belongs to
# the company profile. This is evidence only; it never bypasses registration.
RULE_URLS["pubdisc_orgpage_ru"] = [
    "https://www.orgpage.ru/Cabinet/Create/",
    "https://orgpage.usedocs.com/article/64533",
]
RULE_URLS["pubdisc_yell_ru"] = [
    "https://www.yell.ru/company/create/",
    "https://www.yell.ru/doc/help/",
]
RULE_URLS["pubdisc_saashub_com"] = [
    "https://www.saashub.com/services/submit",
]


def _publication_discovery_authoring_evidence(platform_key: str, inspections: list[dict]) -> dict:
    """Return concrete same-domain authoring evidence for pubdisc candidates.

    Discovery is only a lead source. This gate requires either the candidate URL
    itself or a link exposed by the audited page to point to a recognisable
    authoring route on the same domain.
    """
    if not platform_key.startswith("pubdisc_"):
        return {"found": False, "candidate": None, "authoring_url": None}
    try:
        from app.services import publication_discovery
        candidate = next(
            (
    ...
    urls = []
    for item in inspections:
        urls.extend(str(x or "") for x in (item.get("outbound_links") or []))
    # Prefer a concrete authoring link exposed by the audited page. The search
    # result URL is only a fallback because it may be an explanatory landing.
    urls.append(str(candidate.get("url") or ""))
    for value in urls:
        if not value:
            continue
        try:
            ...
            continue
        # Keep the rule gate on exactly the same URL semantics as discovery.
        # This avoids both false positives such as /add-keyboard and false
        # negatives such as /item/add after the detector is tightened.
        route_score, _ = publication_discovery._authoring_score(value, "", "")
        if host == domain and (route_score >= 4 or "/new/" in route):
            return {"found": True, "candidate": candidate, "authoring_url": value}
    return {"found": False, "candidate": candidate, "authoring_url": None}
...
def inspect_platform(platform_key: str) -> dict:
    ...
    for url in urls:
        ...
        if http_result is None or http_result.get("decision") == "review":
            try:
                browser_result = inspect_url_browser(url, platform_key=platform_key)
    ...
    # JOOMLAFORUM_COMMERCIAL_SECTION_V1
    # The global rules ban advertising/commercial topics in ordinary sections,
    # but explicitly exempt specially designated sections. The audited target
    # board is under «Коммерческие вопросы → Ищу работу» and visibly contains
    # service offers for site development/IT work. Fail closed unless both the
    # global exception and the exact commercial board are present together.
    if platform_key == "joomlaforum_jobs":
        board_excerpts = " ".join(
            str(x.get("text_excerpt") or "").lower()
            for x in inspections
            if "board,29.0" in str(x.get("url") or "")
        )
        rules_excerpts = " ".join(
            str(x.get("text_excerpt") or "").lower()
            for x in inspections
            if "topic,17.0" in str(x.get("url") or "")
        )
        dedicated_board = (
            "коммерческие вопросы" in board_excerpts
            and "ищу работу" in board_excerpts
            and (
                "разработаю сайт" in board_excerpts
                or "разработка и оптимизация сайтов" in board_excerpts
                or "предлагаю свои услуги" in board_excerpts
            )
        )
        explicit_exception = (
            "любую рекламную" in rules_excerpts
            and "коммерческую информацию" in rules_excerpts
            and "за исключением специально отведенных" in rules_excerpts
        )
        duplicate_guard = "не является повторной" in rules_excerpts
        if dedicated_board and explicit_exception and duplicate_guard:
            final = "allowed"
            requirements = sorted(set(requirements) | {
                "use_verified_commercial_jobs_board_only",
                "no_duplicate_service_topic",
                "russian_language_required",
            })
        else:
            final = "review"
    ...
    # PUBLICATION_DISCOVERY_CANONICAL_AUTHORING_GATE_V4
    if platform_key.startswith("pubdisc_"):
        authoring = _publication_discovery_authoring_evidence(platform_key, inspections)
        if not authoring.get("found"):
            final = "review"
            requirements = sorted(set(requirements) | {"exact_same_domain_authoring_surface_required"})
        else:
            # A stale requirement from an older detector must not survive after
            # current evidence proves the exact authoring route. Registration,
            # consent and CAPTCHA remain separate fail-closed checkpoints.
            requirements = sorted(
                x for x in set(requirements)
                if x != "exact_same_domain_authoring_surface_required"
            )
    ...
    # GDE_FREE_LISTING_MANDATORY_LINK_CONFLICT_V1_CANONICAL
    if platform_key == "pubdisc_gde_ru":
        gde_text = " ".join(str(x.get("text_excerpt") or "").lower() for x in inspections)
        if (
            "ссылка на сайт (платно)" in gde_text
    ...
    if platform_key == "pubdisc_saashub_com":
        page_text = " ".join(str(x.get("text_excerpt") or "").lower() for x in inspections)
        if "software development agencies" in page_text and "not accepted" in page_text:
            final = "blocked"
            requirements = sorted(set(requirements) | {
                "software_development_agencies_not_accepted",
                "custom_development_offer_incompatible_with_product_directory",
...
RULE_URLS["disc_forumweb_hosting"] = [
    "https://forumweb.hosting/forums/",
    "https://forumweb.hosting/register/",
]
...
def _fast_it_marketplace_rule_override(platform_key, inspections, final, requirements):
    ...
    if platform_key == "disc_forumweb_hosting":
        # Basic Free does not include Marketplace posting privileges; those
        # privileges are advertised only for paid Premium/Corporate accounts.
        paid_marketplace_gate = (
            "choose account type" in excerpts
            and "basic free" in excerpts
            and "premium membership" in excerpts
            and "corporate membership" in excerpts
            and "access to marketplace posting privileges" in excerpts
        )
        if paid_marketplace_gate:
            return "blocked", sorted(set(requirements) | {
                "paid_membership_required_for_marketplace_posting",
                "basic_free_has_no_marketplace_posting_privileges",
            })
        return "review", requirements
