...
def _orgpage_bulk_directory_results(niche: str, regions: list[str], *, max_results: int = 12,
    ...
    # pagination by crawling unbounded links. Cement currently has six useful
    # pages, while the other bulk categories expose one or two.
    category_pages={
        'postavschiki-sypuchikh':2,
        'schebenochnye-zavody':1,
    ...
    # Rotate each category through its verified finite page depth. This fixes
    # starvation after pages 1-2 were exhausted without increasing requests per
    # pass: still only two category pages are fetched.
    page_round=(raw_offset // len(categories))
    ordered=categories[cat_start:]+categories[:cat_start]
    for cat in ordered[:2]:
        _respect_search_interval()
        effective_page=1+(page_round % int(category_pages.get(cat,1)))
        category_url=(
    ...
    if not cards:
        return []
    start=(raw_offset // len(categories)) % len(cards)
