...
# Runtime imports come from immutable release directories; mutable platform
# state belongs to the canonical writable backend state root.
ROOT = Path(os.environ.get("BORIS_STATE_BACKEND_ROOT") or Path(__file__).resolve().parents[2])
...
ACCOUNT_SECRETS_FILE = STATE_DIR / "account_secrets.json"
BORIS_SITE = "https://boris-ai.pro"

_SECRET_REDACTION_TOKEN = "[REDACTED_SECRET]"

def redact_sensitive_text(value: str | None) -> str | None:
    """Remove credentials from diagnostic strings before they reach state/log files.

    Playwright includes values passed to Locator.fill() in timeout call logs.
    That makes an otherwise harmless browser traceback capable of persisting a
    password. Redact both known stored passwords and generic fill()/password
    patterns. The account secret file itself remains the only credential store.
    """
    if value is None:
        return None
    text = str(value)
    try:
        rows = _load(ACCOUNT_SECRETS_FILE, [])
    except Exception:
        rows = []
    for row in rows if isinstance(rows, list) else []:
        secret = str((row or {}).get("password") or "")
        if len(secret) >= 6:
            text = text.replace(secret, _SECRET_REDACTION_TOKEN)
    text = re.sub(
        r'(?is)fill\((?:"[^"\n]*"|\'[^\'\n]*\')\)',
        'fill("[REDACTED_SECRET]")',
        text,
    )
    text = re.sub(
        r'(?i)((?:password|passwrd|passwd)\s*[=:]\s*)([^\s,;&]+)',
        r'\1[REDACTED_SECRET]',
        text,
    )
    return text
...
def redact_sensitive_value(value):
    """Recursively sanitise diagnostic payloads before persistence.

    Redaction is both value-aware and key-aware. A raw credential stored under
    ``filled.password`` does not contain the word ``password`` inside the value,
    so text-only scrubbing is insufficient. Secret-like mapping keys are
    therefore replaced before descending into their value.
    """
    ...
    if isinstance(value, dict):
        ...
        for key, item in value.items():
            key_text = str(key)
            sensitive_key = bool(re.search(
                r"(?i)(?:^|[_-])(?:password|passwrd|passwd|pwd|secret|api[_-]?key|access[_-]?token|refresh[_-]?token|auth[_-]?token)(?:$|[_-])",
                key_text,
            ))
            if sensitive_key and isinstance(item, str) and item.strip():
                out[key] = _SECRET_REDACTION_TOKEN
            else:
                out[key] = redact_sensitive_value(item)
        return out
    if isinstance(value, list):
        return [redact_sensitive_value(v) for v in value]
    if isinstance(value, tuple):
        return tuple(redact_sensitive_value(v) for v in value)
    return value
...
def _generate_drafts_legacy_v1(platform="all",topic="all",variants=3):
...
def _save_drafts_legacy_v1(drafts):
...
def _mark_posted_legacy_v1(draft_id,url):
...
def _registration_plan_legacy_v1(email: str | None = None) -> list[dict]:
...
def upsert_verified_external_publication(
    *,
    platform: str,
    url: str,
    target_url: str,
    ...
    verification=None,
    source: str = "guest_publication",
) -> dict:
    """Idempotently persist a verified publication produced outside a draft adapter.

    Guest boards and catalog adapters can produce valid public URLs without a
    marketplace draft row. Keeping those proofs in sidecar JSON made the owner
    reconcile several ledgers manually and allowed later cycles to forget an
    already-published domain. A verified external result belongs in the same
    canonical publications ledger as ordinary forum posts.
    """
    ...
    if not url.startswith(("https://", "http://")):
        raise ValueError("absolute publication url required")
    if not target_url.startswith(("https://", "http://")):
        raise ValueError("absolute target url required")

    rows = _load(PUBLICATIONS_FILE, [])
    canonical = url.rstrip("/")
    existing = next(
        (row for row in rows if str(row.get("url") or "").rstrip("/") == canonical),
        None,
    )
    now = datetime.now(timezone.utc).isoformat()
    if existing is not None:
        existing["verified"] = True
        existing["verified_at"] = verified_at or existing.get("verified_at") or now
        existing["site_url"] = existing.get("site_url") or target_url
        ...
        _save(PUBLICATIONS_FILE, rows)
        result = dict(existing)
        result["created"] = False
        return result

    safe_platform = re.sub(r"[^a-zA-Z0-9_.-]+", "_", platform).strip("_") or "external"
    digest = hashlib.sha256(canonical.encode("utf-8")).hexdigest()[:12]
    item = {
        "draft_id": f"external_{safe_platform}_{digest}",
        "platform": platform,
        "topic": "software_development_crowd",
        "url": url,
        "site_url": target_url,
        "posted_at": posted_at or verified_at or now,
        "verified": True,
...
def update_external_publication_verification(
    url: str,
    *,
    verified: bool,
    ...
    """Persist the latest live verification state for an external publication."""
    url = str(url or "").strip()
    if not url.startswith(("https://", "http://")):
        raise ValueError("absolute publication url required")
    rows = _load(PUBLICATIONS_FILE, [])
    canonical = url.rstrip("/")
    row = next(
        (item for item in rows if str(item.get("url") or "").rstrip("/") == canonical),
        None,
    )
    if row is None:
        raise KeyError(url)
    row["verified"] = bool(verified)
    row["verification_checked_at"] = checked_at or datetime.now(timezone.utc).isoformat()
    if link_present is not None:
        row["link_present"] = bool(link_present)
    if verified:
        row.pop("verification_error", None)
    else:
        row["verification_error"] = str(error or "target_link_missing_or_not_indexable")[:500]
    _save(PUBLICATIONS_FILE, rows)
    return dict(row)
...
def _generate_drafts_platform_style_v2(platform="all", topic="all", variants=3):
...
def _platform_policy_static_v1(key: str) -> dict:
    return dict(FREE_PLATFORM_POLICY.get(key, {"free": False, "reason": "not_verified"}))

def _list_platforms_legacy_v2():
    items = []
...
_platform_style_generator = _generate_drafts_platform_style_v2

def _generate_drafts_free_only_legacy_v3(platform="all", topic="all", variants=3):
...
_more_platforms = [
    ...
    Platform("joomlaforum_jobs","JoomlaForum · Ищу работу","https://joomlaforum.ru/index.php/board,29.0.html","direct_post",35,
             "IT, веб-разработка, сайты, программирование, услуги","обычная регистрация",
             "Только раздел «Коммерческие вопросы → Ищу работу»; общие разделы форума для рекламы запрещены."),
...
for _k in [
    "stroy_forum","ssa","towerbuild","forum_baza_1c","bitrix_dev","homeidea","joomlaforum_jobs",
...
_static_platform_policy = _platform_policy_static_v1

def _platform_policy_dynamic_legacy_v2(key: str) -> dict:
...
def record_attempt(
    *,
    ...
    item = {
        ...
        "meta": redact_sensitive_value(meta or {}),
...
def validate_publication_content(draft: dict) -> dict:
    """Global content/landing guard used by every publication channel."""
    title=str(draft.get("title") or "")
    body=str(draft.get("text") or "")
    target=str(draft.get("target_url") or "").strip()
    combined=f"{title}\n{body}".casefold()

    paragraphs=[x.strip() for x in re.split(r"\n\s*\n", body) if x.strip()]
    last=paragraphs[-1] if paragraphs else ""
    m=re.match(r"^(?:Ключевые фразы|Keyword phrases):\s*(.+)$", last, re.I|re.S)
    if not m:
        return {"ok":False,"reason":"keyword_block_not_last"}
    keywords=[x.strip().strip(".") for x in m.group(1).split(";") if x.strip()]
    if not (10 <= len(keywords) <= 20):
        return {"ok":False,"reason":"keyword_count_invalid","keyword_count":len(keywords)}

    product_markers=(
        "готовые модули","готовая crm","ии-менеджер","автозагрузка avito",
        "автоматизация avito","яндекс директ","реактивация клиентской базы",
        "коллтрекинг","сквозная аналитика","мультиаккаунт","панель руководителя",
    )
    looks_product=any(x in combined for x in product_markers)
    looks_development=any(x in combined for x in (
        "разработ","под заказ","создаём","создаем","создание собственной",
        "кастомн","mvp","web app","saas","api-интеграц","api интеграц",
        "development","custom development","integration development",
    ))

    if target.rstrip("/") == "https://boris-ai.pro/software-dev":
        return {"ok":False,"reason":"generic_software_dev_landing_forbidden"}

    if "/software-dev/services/" in target:
        if looks_product and not looks_development:
            return {"ok":False,"reason":"product_copy_on_development_landing"}
        if not looks_development:
            return {"ok":False,"reason":"development_landing_without_development_intent"}
        return {"ok":True,"family":"development","keyword_count":len(keywords)}

    if target.rstrip("/") == "https://boris-ai.pro":
        if looks_development and not looks_product:
            return {"ok":False,"reason":"development_copy_on_product_landing"}
        return {"ok":True,"family":"product","keyword_count":len(keywords)}

    return {"ok":False,"reason":"unsupported_target_landing","target":target}
...
def mark_posted(draft_id: str, url: str) -> dict:
    ...
    content_gate = validate_publication_content(draft)
    if not content_gate.get("ok"):
        record_attempt(
            ...
            status_value="blocked",
            draft_id=draft_id,
            error_code=str(content_gate.get("reason") or "content_landing_guard"),
            error_detail="Глобальный SEO guard: смысл, landing или блок ключей не соответствуют железным правилам публикации.",
        )
        raise PermissionError(str(content_gate.get("reason") or "content_landing_guard"))
...
# FRESH_POLICY_V5
def _platform_policy_fresh_legacy_v5(key: str) -> dict:
    base = dict(FREE_PLATFORM_POLICY.get(key, {"free": False, "reason": "not_verified"}))
    rule_path = STATE_DIR / "rules" / f"{key}.json"
...
_DYNAMIC_SURFACE_SERVICE_MARKERS = (
    ...
    "freelance projects", "вакансии и проекты", "проекты для фрилансеров",
...
def _discovered_platform_objects() -> list[Platform]:
    ...
    for p in PLATFORMS:
        try:
            ...
            observe_suppressed(__name__, _suppressed_exc, line=1371)
    out: list[Platform] = []
    base_priority = 1000
...
def _publication_discovered_platform_objects() -> list[Platform]:
    """Strong non-forum publication candidates discovered at scale.

    Only Yandex-IKS medium/high domains are admitted here. They still require a
    rule audit before outreach is enabled; discovery alone never authorizes a
    post.
    """
    try:
        from app.services import publication_discovery
        rows = list((publication_discovery.load() or {}).get("items") or [])
    except Exception:
        return []
    ...
    for p in PLATFORMS:
        try:
            ...
            observe_suppressed(__name__, _suppressed_exc, line=1421)
    ...
    for idx, row in enumerate(rows):
        ...
        if duplicate_static:
            continue
        if str(row.get("iks_tier") or "") not in {"high", "medium"}:
            continue
        ...
        safe_domain = re.sub(r"[^a-z0-9]+", "_", domain).strip("_")[:48]
        key = f"pubdisc_{safe_domain}"
        out.append(Platform(
            key=key,
            name=str(row.get("title") or domain)[:160],
            url=url,
            mode="direct_post",
            priority=base_priority + idx,
            audience="Автоматически найдено: services",
            registration="автоматическая разведка регистрации/публикации",
            notes=(
                f"Publication Discovery: {kind}; "
                f"Yandex IKS={row.get('iks')} ({row.get('iks_tier')}). "
                "Сначала обязательный rule-audit собственной страницы/объявления."
            ),
        ))
    return out
...
def all_platform_objects() -> list[Platform]:
    ...
    for p in _publication_discovered_platform_objects():
        merged.setdefault(p.key, p)
...
_EXPLICIT_FORUM_KEYS = {
    "wmboard_services",
    "webledi_services",
    # These forum roots do not expose the word "forum" in the URL, so the
    # generic URL/name heuristic cannot classify them safely on its own.
    "homeidea",
    "itnull_services",
}
...
VERIFIED_PUBLICATION_SURFACES = {
    "joomlaforum_jobs": [
        {
            "id": "commercial_jobs",
            "name": "Коммерческие вопросы · Ищу работу",
            "url": "https://joomlaforum.ru/index.php/board,29.0.html",
            "post_url": "https://joomlaforum.ru/index.php?action=post;board=29.0",
            "niches": ["it", "services", "business"],
            "required_terms": [],
            "language": "ru",
            "evidence": "Общие правила запрещают коммерческую информацию кроме специально отведённых разделов; этот раздел находится в «Коммерческие вопросы» и содержит предложения разработки сайтов и IT-услуг.",
        },
    ],
    "forum_baza_1c": [
        {
    ...
    "forum_seo_net": [
        ...
        {
            "id": "design_technical_services",
            "name": "Дизайн и Техническая часть",
            "url": "https://forum-seo.net/forums/dizain-i-tekhnicheskaya-chast.50/",
            "post_url": "https://forum-seo.net/forums/dizain-i-tekhnicheskaya-chast.50/post-thread",
            "niches": ["it", "development", "services"],
            "required_terms": ["разработ", "программ", "crm", "saas", "api", "web", "веб", "сайт"],
            "evidence": "Живой раздел форума описан как разработка сайтов под ключ, правка кода, верстка и техническая реализация; подходит для предложения разработки ПО.",
        },
    ],
    "foodmarkets": [
        {
...
def _publication_discovery_surface(candidate: dict) -> list[dict]:
    """Convert only an exact own-listing route into the existing placement model."""
    if not candidate:
        return []
    url = str(candidate.get("url") or "").strip()
    kind = str(candidate.get("kind") or "").strip().lower()
    if not url or kind not in {"catalog", "classified", "services", "ugc_article"}:
        return []
    parsed = urlparse(url)
    ...
    own_listing_markers = (
        "добавить компанию", "разместить компанию", "добавить организацию",
        "подать бесплатное объявление", "разместить объявление бесплатно",
        "add company", "add business", "create listing", "submit listing",
    )
    author_route_markers = (
        "/company/create", "/cabinet/create", "/add-company", "/add_company",
        "/post", "/add-listing", "/create-listing", "/submit-listing",
    )
    if not (
        any(marker in signal for marker in own_listing_markers)
        and any(marker in route for marker in author_route_markers)
    ):
        return []
    return [{
        "id": "publication_discovery_own_listing",
        "name": str(candidate.get("title") or parsed.netloc)[:160],
        "url": url,
        "post_url": url,
        "niches": ["it", "business", "services"],
        "required_terms": [],
        "language": "ru",
        "evidence": "Publication Discovery: exact same-domain own-listing route; rule audit still required.",
    }]
...
def list_platforms():
    ...
    try:
        ...
        for candidate in (publication_discovery.load() or {}).get("items", []):
            domain = str(candidate.get("domain") or "").lower().removeprefix("www.")
            if not domain:
                continue
            safe_domain = re.sub(r"[^a-z0-9]+", "_", domain).strip("_")[:48]
            publication_candidates[f"pubdisc_{safe_domain}"] = candidate
    except Exception:
        publication_candidates = {}
    publication_keys = set(publication_candidates)
    ...
    for p in all_platform_objects():
        ...
        row["free_policy"] = platform_policy(p.key)
        row["enabled_for_outreach"] = bool(row["free_policy"].get("free"))
        row["discovered_automatically"] = p.key in dynamic_keys or p.key in publication_keys
        ...
        # PUBLICATION_READY_RESPECTS_REGISTRATION_CHECKPOINT_V1
        # Non-forum direct-post surfaces may legitimately support guest posting,
        # but once registration discovery has found an unresolved CAPTCHA,
        # terms/SMS/SSO/phone checkpoint they are not publication-ready.
        # Forum accounts remain strict: they must be fully ready.
        requires_ready_account = bool(
            row["channel_type"] == "forum" or p.key in publication_keys
        )
        row["publication_ready"] = bool(
            ...
            and (
                ...
                or (
                    not requires_ready_account
                    and not row["registration_checkpoint"]
                )
            )
        )
        ...
        if (
            not row["publication_surfaces"]
            and p.key in publication_keys
            and row["free_policy"].get("rule_decision") == "allowed"
        ):
            row["publication_surfaces"] = _publication_discovery_surface(
                publication_candidates.get(p.key) or {}
            )
        seo = _seo_for_platform(p.key, p.audience)
        row["seo_topics"] = seo["topics"]
        row["seo_keywords"] = seo["keywords"]
...
def registration_plan(email: str | None = None) -> list[dict]:
    ...
    for p in all_platform_objects():
        ...
        out.append({
            ...
            "notes": p.notes,
            "discovered_automatically": p.key.startswith(("disc_", "pubdisc_")),
        })
    return out
...
HUMAN_PLATFORM_BOOTSTRAP_CHECKPOINTS = {
    ...
    "captcha_age_declaration_required": "Один раз пройти CAPTCHA и самостоятельно указать требуемый площадкой возраст/дату рождения",
    ...
    "age_declaration_required": "Один раз подтвердить требуемый площадкой возрастной статус",
    ...
    "truthful_company_identity_required": "Один раз указать подтверждённые реальные данные компании для карточки",
}

# HUMAN_CHECKPOINT_HANDOFF_V1
# Разделяем шаги, которые требуют открытого браузера, и шаги, где человек уже
# получил короткое значение (SMS/OTP). BORIS не обходит антибот-защиту, но
# сохраняет точку продолжения и после подтверждения автоматически возвращает
# площадку в обычный verify -> READY/WARMING -> Crowd SEO поток.
INTERACTIVE_BROWSER_CHECKPOINTS = {
    "manual_verification",
    "captcha_required", "captcha_age_and_terms_required", "captcha_age_declaration_required",
    "terms_acceptance_required", "registration_agreement_required",
    "age_and_terms_declaration_required", "age_declaration_required",
...
TEXT_VALUE_CHECKPOINTS = {
    "sms_or_verification_code_required", "phone_sms_required",
}
...
def platform_bootstrap_queue() -> dict:
    ...
    platforms = {
        ...
        if x.get("channel_type") == "forum" or bool(x.get("publication_surfaces"))
    }
    ...
    for key, platform in platforms.items():
        ...
        surfaces = list(
            platform.get("publication_surfaces")
            or VERIFIED_PUBLICATION_SURFACES.get(key, [])
            or []
        )
        niches = {
            str(n).strip().lower()
        ...
        items.append({
            ...
            "verification_module_required": True,
            "executor_role": "platform_onboarding_worker",
            "one_time": True,
            "requires_interactive_browser": checkpoint in INTERACTIVE_BROWSER_CHECKPOINTS,
            "accepts_text_value": checkpoint in TEXT_VALUE_CHECKPOINTS,
    ...
    return {
        ...
        "verification_module_required": bool(items),
        "verification_module_tasks": len(items),
        "interactive_browser_tasks": sum(1 for x in items if x.get("requires_interactive_browser")),
        "text_value_tasks": sum(1 for x in items if x.get("accepts_text_value")),
        "reusable_across_clients": True,
        "executor_role": "platform_onboarding_worker",
