...
def _search_bing(query: str, limit: int = 10, request_timeout_s: float = 20) -> list[dict]:
    url = "https://www.bing.com/search?count=20&q=" + quote(query)
    r = requests.get(url, headers={"User-Agent": UA, "Accept-Language": "ru-RU,ru;q=0.9"}, timeout=request_timeout_s)
    r.raise_for_status()
...
def _search_ddg_lite(query: str, limit: int = 10, request_timeout_s: float = 20) -> list[dict]:
...
def _search_all(
    ...
    *,
    ...
    with ThreadPoolExecutor(max_workers=len(engines)) as pool:
        futures = {
            pool.submit(fn, query, limit, request_timeout_s): name
