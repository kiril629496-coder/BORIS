...
def _tg(text_msg):
    ...
    except Exception as _suppressed_exc: observe_suppressed(__name__, _suppressed_exc, line=21)

def _find_inbox_message(mailbox_id:int,message_id:str)->bool:
    if not message_id:return False
    from app.services.client_mailboxes import _row, _mailbox_password
    import imaplib
    r=_row(mailbox_id)
    ...
    try:
        ...
        try:m.logout()
        except Exception as _suppressed_exc:observe_suppressed(__name__, _suppressed_exc, line=37)
...
def tick()->dict:
    ...
    try:
        ...
        mb=int(mb)
        if not row or row['mailbox_id']!=mb or str(row.get('email_address') or '').strip().lower()!=EMAIL:
            ...
            q=enqueue_email(EMAIL,'BORIS TEST — почта и «Отправленные» работают',body,html=html,source='prospect_self_test',idempotency_key=f'prospect_self_test:{OWNER_ID}:{mb}:{EMAIL}',send_now=False,attachments=attachments,mailbox_id=mb)
        ...
        if state in ('self_queued','self_sent'):
            ...
            if q['status']=='dead':
                db.execute(text("UPDATE prospect_outreach_bootstrap SET state='self_failed',last_error=:e,updated_at=NOW() WHERE owner_id=:o"),{'e':q['last_error'],'o':OWNER_ID});db.commit()
                _tg('❌ <b>BORIS: контрольный тест почты не прошёл</b>\n\nРассылка не запущена.')
                return {'state':'self_failed'}
            if q['status']=='sent':
                mid=q['provider_message_id'] or ''
                # SMTP/IMAP auth were already proven by test_mailbox(). A real
                # queue status=sent proves SMTP delivery, while the Sent-copy
                # recovery queue proves whether IMAP append still needs repair.
                # Some providers (including Yandex) do not mirror self-mail into
                # INBOX, so Inbox presence is optional evidence, never a blocker.
                ...
                if pending:
                    return {'state':'waiting_sent_copy'}
                inbox_seen=False
                try:
                    inbox_seen=bool(_find_inbox_message(mb,mid))
                except Exception:
                    inbox_seen=False
                ...
                _tg('✅ <b>BORIS: контрольный email-тест PASS</b>\n\nПроверено: SMTP, IMAP, реальная отправка и копия в «Отправленных». Почтовый контур готов. Активные owner-outreach кампании работают только по своей явной конфигурации: ramp 10→20/сутки, интервал ≥20 минут. Черновики автоматически не активируются.')
                return {'state':'active','inbox_seen':inbox_seen}
        return {'state':state}
    finally: db.close()
