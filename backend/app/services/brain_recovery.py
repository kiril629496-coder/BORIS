    ...
    for t in scope_rows:
        ...
        if reason is None:
            ...
            try:
                ...
                cfg={}
            wl={str(x) for x in ((cfg or {}).get('item_ids') or []) if str(x)} if isinstance(cfg,dict) else set()
...
def _safe_crm_overdue_queue_recovery(db) -> dict:
    ...
    return {
      ...
      "result":{
        "reminded":reminded,"eligible_rows":len(rows)-skipped_inactive,
