...
def _mop_high_confidence_first_turn_reply(question: str, history_text: str) -> str | None:
    ...
    if _hf_re.search(r"(?:интересует|нужна|хочу).{0,25}(?:ипотек|рефинанс)", low):
        return "Подскажите, пожалуйста, что именно хотите решить: одобрение ипотеки, рефинансирование или покупку недвижимости?"
    return None
...
def _mop_emergency_next_question(required_questions, history_text: str) -> str:
    ...
    def _known(question: str) -> bool:
        ...
        if "телефон" in q or "номер" in q or "контакт" in q:
            return bool(
                _eq_re.search(r"(?<!\d)(?:\+?7|8)[\s()\-]*\d{3}[\s()\-]*\d{3}[\s\-]*\d{2}[\s\-]*\d{2}(?!\d)|(?<!\d)9\d{9}(?!\d)", client_text)
...
def _mop_echo_repair_reply(history_text: str) -> str:
    ...
    if _echo_re.search(r"(?:подобрат|подбор).{0,35}(?:квартир|недвиж)|(?:квартир|недвиж).{0,35}(?:подобрат|подбор)", low):
        return "Начнём с основных параметров. Какой бюджет рассматриваете?"
    if _echo_re.search(r"(?:ипотек|рефинанс|кредит.{0,20}залог)", low):
        return "Подскажите, пожалуйста, что именно хотите решить: одобрение ипотеки, рефинансирование или покупку недвижимости?"
...
def generate_ai_draft_reply(account_id: str, chat: dict, proactive_event: str = None,
    ...
    if _high_confidence_reply:
        ...
        return _wrap(_high_confidence_reply, _fast_usage, return_meta, analysis=_fast_analysis)
    ...
    try:
            ...
            if cached:
                ...
                if _cached_policy:
                    ...
                    if "phone_already_received" in _cpv:
                        ...
                        _next_after_repeat = _mop_emergency_next_question(_required_questions, history_text)
                        _cached_text = _next_after_repeat or "Принял. Этот момент уже зафиксирован, повторно уточнять его не буду."
                        ...
                        _cached_text = "Принял уточнение. Этот шаг уже зафиксирован, повторять его не буду."
                    ...
                    _continuity_only = all(
                        x in {"client_echo", "phone_already_received", "repeated_handoff_promise", "measurement_required_no_dimensions"}
        ...
        if _vision_urls:
            ...
            try:
                ...
                if (
                    ...
                    _phone_known = bool(
                        _emergency_re.search(r"(?<!\d)(?:\+?7|8)[\s()\-]*\d{3}[\s()\-]*\d{3}[\s\-]*\d{2}[\s\-]*\d{2}(?!\d)|(?<!\d)9\d{9}(?!\d)", _all_client_low)
        ...
        if _policy_violations:
            ...
            if "client_echo" in _pv:
                ...
                _next_after_repeat = _mop_emergency_next_question(_required_questions, history_text)
                reply_text = _next_after_repeat or "Принял. Этот момент уже зафиксирован, повторно уточнять его не буду."
                ...
                reply_text = "Принял уточнение. Этот шаг уже зафиксирован, повторять его не буду."
            ...
            _continuity_only = all(
                x in {"phone_already_received", "repeated_handoff_promise", "measurement_required_no_dimensions"}
                or x.startswith("repeated_answered_question:")
                for x in _pv
            )
            analysis.update({
