...
def _resume_crowd_after_ready(platform: str) -> dict:
    """Hand off post-verification continuation to the canonical systemd guardian."""
    try:
        proc = subprocess.run(
            ["systemctl", "start", "--no-block", "boris-forum-acquisition-guard.service"],
            capture_output=True,
            text=True,
            timeout=5,
            check=False,
        ...
        return {
            "status": "deferred",
            "reason": f"guardian_schedule_error:{type(exc).__name__}",
            "platform": platform,
        }
    if proc.returncode != 0:
        return {
            "status": "deferred",
            "reason": ((proc.stderr or proc.stdout or "guardian_start_failed")[-500:]).strip(),
            "platform": platform,
        }
    return {
        "status": "scheduled",
        "platform": platform,
        "executor": "boris-forum-acquisition-guard.service",
    }
...
def _inventory_summary() -> dict:
    ...
    for fmt in reserve_policy.SUPPORTED_FORMATS:
        active_target = int(active_format_targets.get(fmt) or 0)
        strategic_target = int(reserve_policy.STRATEGIC_PER_FORMAT_TARGET)
        target = max(active_target, strategic_target)
        available = len(sellable_format_sites.get(fmt, set()))
        sellable_product_formats[fmt] = {
            "target": target,
            "active_order_target": active_target,
            "strategic_reserve_target": strategic_target,
            "allowed_unique_sites": available,
            "deficit": max(0, target - available),
            "sellable": available >= target,
            "target_source": "max(active_client_order, strategic_reserve_floor)",
        }
...
def verify_bootstrap(platform: str):
    ...
    if item is None:
        ...
        if current and current.get("publication_ready"):
            return {
                ...
                "resume": _resume_crowd_after_ready(platform),
                "message": "Площадка уже READY. BORIS передал продолжение рабочему циклу.",
                "queue": queue,
            }
    ...
    resume = _resume_crowd_after_ready(platform) if ready else {"status": "not_ready", "platform": platform}
    return {
        "ok": True,
        "platform": platform,
        "ready": ready,
...
@router.post("/bootstrap-queue/{platform}/complete-checkpoint")
def complete_checkpoint(platform: str, req: HumanCheckpointRequest):
    value = req.value.strip()
    if not value:
        raise HTTPException(status_code=400, detail="Введите код или ответ.")
    ...
    try:
        ...
        raise HTTPException(status_code=504, detail="Отправка кода превысила безопасный лимит времени.") from exc
    ...
    if proc.returncode != 0:
        detail = ((proc.stderr or proc.stdout or "")[-1200:] or "complete-checkpoint failed").strip()
        raise HTTPException(status_code=502, detail=detail)

    payload = payload or {}
    checkpoint = payload.get("checkpoint")
    verification = None
    if checkpoint == "interactive_captcha_required":
        message = "Нужна интерактивная CAPTCHA. После её прохождения BORIS продолжит регистрацию."
    elif payload.get("submitted"):
        # CHECKPOINT_IMMEDIATE_RESUME_V1: раньше код принимался, но фактическая
        # проверка аккаунта ждала следующего worker-cycle. Теперь подтверждение
        # сразу проходит через тот же verify path и, при READY, запускает Crowd SEO.
        try:
            verification = verify_bootstrap(platform)
        ...
        if verification and verification.get("ready"):
            message = "Код/ответ принят. Площадка READY, BORIS сразу продолжил рабочий контур."
        else:
            message = "Код/ответ отправлен. BORIS сразу перепроверил площадку; следующий внешний шаг сохранён в очереди."
    else:
        message = "Поле кода на текущем шаге не найдено. BORIS оставил площадку в очереди."
    ...
    return {
        ...
        "message": message,
        "queue": (verification or {}).get("queue") or _prioritized_bootstrap_queue(),
    }
