...
REGISTRATION_URLS = {
    ...
    "joomlaforum_jobs": "https://joomlaforum.ru/index.php?action=register",
    ...
    "disc_forumweb_hosting": "https://forumweb.hosting/register/",
    "disc_teaforum_org": "https://www.teaforum.org/ucp.php?mode=register",
    "disc_forum_arcadecontrols_com": "https://forum.arcadecontrols.com/index.php?action=register",
    "disc_teachat_com": "https://www.teachat.com/ucp.php?mode=register",
    "disc_pbnation_com": "https://www.pbnation.com/register.php",
    "disc_ttforum_co_uk": "https://www.ttforum.co.uk/forum/ucp.php?mode=register",
    "disc_cycleforums_com": "https://www.cycleforums.com/forums/register/",
}
...
def _log(platform: str, action: str, payload: dict) -> Path:
    ...
    # Browser-run artifacts are diagnostics, never credential storage. Sanitize
    # recursively before writing so fields such as filled.password cannot leak.
    _save(p, marketplace_svc.redact_sensitive_value(payload))
    return p
...
OPERATOR_PROFILE = DATA / "operator_profile.json"

def _prefill_account_fields(page, email: str, account: dict) -> dict:
    """Prepare ordinary account/profile fields without submitting the form."""
    profile = _load(OPERATOR_PROFILE, {})
    ...
    user_sel = _fill_first(page, [
        'input[name*="user" i]','input[name*="login" i]','input[autocomplete="username"]'
    ], username)
    if user_sel:
    ...
    if phone:
        phone_sel = _fill_first(page, [
            'input[type="tel"]','input[name*="phone" i]','input[name*="mobile" i]',
            'input[name*="tel" i]','input[id*="phone" i]','input[id*="mobile" i]'
        ], phone)
        if phone_sel:
            filled["phone"] = phone_sel
...
def _registration_policy_checkpoint(page, email: str, account: dict, platform: str | None = None) -> tuple[str | None, str | None]:
    """Block registrations that require data BORIS must not fabricate or consent BORIS must not give."""
    # Some sites keep the legal acceptance statement on a separate rules page,
    # not on the registration form itself. Use the fresh audited snapshot so a
    # visually plain form cannot bypass the no-automatic-consent policy.
    if platform == "joomlaforum_jobs":
        try:
            rule = platform_rules.latest(platform) or {}
            audited_text = " ".join(
            ...
            audited_text = ""
        if (
            "нажимая кнопку вход или регистрация" in audited_text
            and "соглашаетесь на обработку персональных данных" in audited_text
        ):
            return (
                "terms_acceptance_required",
                "Правила JoomlaForum прямо считают нажатие входа/регистрации согласием на обработку персональных данных. BORIS не даёт такое согласие автоматически.",
            )
    # REGISTRATION_EXPLICIT_LEGAL_CONSENT_FAIL_CLOSED_V1:
    # Detect a dedicated legal-consent control before any registration adapter
    # can fill the form. BORIS may inspect a form, but never checks or accepts
    # this agreement on behalf of the owner.
    try:
        if page.locator('input[name="legal_soglasie"]').count():
            return (
                "terms_acceptance_required",
                "Форма содержит отдельное юридическое согласие. BORIS не отмечает и не принимает его автоматически.",
            )
    except Exception:
        pass
    ...
    low = body.lower()
    try:
        coppa_buttons = page.locator('input[type="submit"][name^="coppa_" i], button[name^="coppa_" i]')
        if coppa_buttons.count() >= 1 and ("under the age of 13" in low or "i'm 13+" in low or "i am 13+" in low):
            return (
                "age_declaration_required",
                "Площадка требует выбрать возрастной статус (COPPA). BORIS не делает возрастное заявление автоматически.",
            )
    except Exception:
        pass
    if any(x in low for x in [
    ...
    # Date of birth / age is always a human declaration. BORIS may prepare
    # ordinary credentials, but must never fill or submit age/DOB controls.
    has_birthdate_controls = False
    try:
        has_birthdate_controls = (
            page.locator(
                'input[name*="dob" i],input[name*="birth" i],input[type="date"],'
                'select[name*="dob" i],select[name*="birth" i],'
                'select#regMonth,select#regDay,select#regYear'
            ).count() >= 1
        )
    ...
    if has_birthdate_controls:
        return (
            "age_declaration_required",
            "Площадка требует дату рождения/возраст. BORIS не заполняет и не подтверждает возрастные данные автоматически.",
        )
    ...
    try:
        ...
        for i in range(min(checkboxes.count(), 12)):
            ...
            if any(x in context for x in ["terms", "agree", "consent", "legal", "soglas", "gdpr", "iagree", "правил", "соглаш", "услов"]):
                return (
                    "terms_acceptance_required",
    ...
    try:
        ...
        if agree_submit.count() and agree_submit.is_visible():
            ...
            if any(x in agree_text for x in [
                "соглашаюсь", "согласен", "услови", "правил",
                "i agree", "agree to these terms", "agree to the terms",
            ]):
                return (
                    "terms_acceptance_required",
...
def _external_sso_only_checkpoint(page) -> tuple[str | None, str | None]:
    """Detect registration pages delegated entirely to an external identity provider.

    A page that only offers Telegram/Google/GitHub/vendor SSO is not a broken
    registration form. BORIS must expose a reusable human checkpoint instead of
    repeatedly retrying it as registration_form_not_found. Visible local email
    or new-password fields keep the normal local-registration path available.
    """
    try:
    ...
    if not any(marker in body_low for marker in markers):
        return None, None

    def _has_visible(selector: str) -> bool:
        try:
    ...
    local_registration = any(_has_visible(selector) for selector in (
        'input[type="email"]',
        'input[autocomplete="email"]',
        'input[autocomplete="new-password"]',
        'form[action*="register" i] input[type="password"]',
        'form[action*="signup" i] input[type="password"]',
    ))
    if local_registration:
        return None, None
    return (
        "external_account_sso_required",
        "Площадка создаёт аккаунт только через внешний Telegram/Google/GitHub/SSO. "
        "BORIS не авторизует внешнюю личность автоматически.",
    )
...
def _fill_registration_adapter(page, platform: str, email: str, account: dict) -> dict:
    ...
    if platform == "forum_baza_1c":
        ...
        try:
            ...
            pass
        filled.update({"username": "user", "email": "email", "password": "passwrd1/passwrd2", "profession": "Разработчик 1С"})
        return filled
    ...
    if platform == "foodmarkets":
        ...
        filled.update({"username": "login", "email": "email1/email2"})
        return filled
...
def _registration_http_fallback(platform: str, start_url: str) -> dict | None:
    ...
    try:
        ...
        if (
            platform == "disc_cpu_world_com"
            and response.status_code == 403
            and "this ip address is blocked" in response.text.lower()
        ):
            return {
                "platform": platform,
                "url": response.url,
                "checkpoint": "registration_route_unavailable",
                "detail": (
                    "CPU-World blocks the production host/datacenter IP and automated access "
                    "before registration. This route cannot be completed autonomously from BORIS."
                ),
            }
        response.raise_for_status()
    except Exception:
    ...
    captcha_markers = [
        ...
        "pow_challenge_data", "proof-of-work", "proof of work",
    ]
    checkpoint = None
    detail = None
    if platform == "wmboard_qa":
        registration_links = [
            a.get("href")
            for a in soup.find_all("a", href=True)
            if any(token in (str(a.get_text(" ", strip=True)) + " " + str(a.get("href"))).lower()
                   for token in ("register", "registration", "регистра", "sign up", "signup", "login", "вход"))
        ]
        if soup.find("form") is None and not registration_links:
            return {
                "platform": platform,
                "url": response.url,
                "checkpoint": "registration_route_unavailable",
                "detail": (
                    "QA.WMBoard exposes the public forum but no registration/login authoring route; "
                    "standard /register and /login endpoints are unavailable. Treat as unavailable "
                    "instead of retrying registration_form_not_found indefinitely."
                ),
            }
    if platform == "pubdisc_orgpage_ru":
        company_fields = {
            str(tag.get("name") or "")
            for tag in soup.find_all(["input", "textarea", "select"])
        }
        company_card_form = bool(
            {"name", "shortDesc", "fullDesc"} <= company_fields
            and "добавьте вашу компанию в каталог" in text
        )
        if company_card_form:
            return {
                "platform": platform,
                "url": response.url,
                "checkpoint": "truthful_company_identity_required",
                "detail": (
                    "Orgpage company-card form is reachable over ordinary HTTP, "
                    "but it requires verified real company OPF/name/city/address/phone. "
                    "No values were submitted."
                ),
            }
    coppa_submit = soup.find(["input", "button"], attrs={"name": re.compile(r"^coppa_", re.I)})
    if coppa_submit is not None and ("under the age of 13" in text or "i'm 13+" in text or "i am 13+" in text):
        checkpoint = "age_declaration_required"
        detail = "Площадка требует выбрать возрастной статус (COPPA). BORIS не делает возрастное заявление автоматически."
    elif any(marker in html or marker in text for marker in captcha_markers):
        checkpoint = "captcha_required"
        ...
        agree_submit = soup.find(["input", "button"], attrs={"name": "agreed"})
        agree_text = ""
        if agree_submit is not None:
            agree_text = " ".join([
                str(agree_submit.get("value") or ""),
                " ".join(agree_submit.stripped_strings),
            ]).lower()
        explicit_agreement_submit = any(marker in agree_text for marker in [
            "i agree", "agree to these terms", "agree to the terms",
            "соглашаюсь", "согласен",
        ])
        if (
            (checkboxes and any(marker in text for marker in terms_markers))
            or any(marker in text for marker in implied_terms_markers)
            or explicit_agreement_submit
        ):
            checkpoint = "terms_acceptance_required"
...
def _registration_status_for_checkpoint(checkpoint: str | None) -> str:
    if checkpoint in {
        ...
        return "blocked"
    if checkpoint in {
        "captcha_required", "captcha_age_and_terms_required", "captcha_age_declaration_required", "sms_or_verification_code_required",
        "email_verification_required", "age_and_terms_declaration_required", "age_declaration_required",
...
def register(platform: str, email: str | None, *, headless: bool, submit: bool) -> dict:
    ...
    if platform in {"searchengines_services", "pubdisc_orgpage_ru"}:
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            if fallback is None:
                ...
                status = "in_progress"
                detail = (
                    "Временная ошибка открытия регистрационной страницы; "
                    "BORIS повторит безопасный preflight автоматически. "
                    f"{type(exc).__name__}: {str(exc)[:500]}"
                )
                marketplace_svc.upsert_registration(
                    platform, status, email=email, account_url=start_url,
                    checkpoint=checkpoint, last_error=detail,
                )
                marketplace_svc.record_attempt(
                    platform=platform, action="register", status_value="failed",
                    url=start_url, error_code=checkpoint, error_detail=detail,
                    meta={"submitted": False, "filled": {}, "http_fallback": False, "transient": True},
                )
                return {
                    "ok": False, "platform": platform, "url": start_url,
                    "filled": {}, "submitted": False, "checkpoint": checkpoint,
                    "http_fallback": False, "transient": True,
                }
            checkpoint = str(fallback.get("checkpoint") or "registration_form_not_found")
            status = _registration_status_for_checkpoint(checkpoint)
            detail = str(fallback.get("detail") or "")
        ...
        direct_company_card = False
        if platform == "pubdisc_orgpage_ru":
            try:
                body_low = page.locator("body").inner_text(timeout=3000).lower()
                direct_company_card = bool(
                    "/cabinet/create" in page.url.lower()
                    and "добавьте вашу компанию в каталог" in body_low
                    and "название компании" in body_low
                )
            except Exception:
                direct_company_card = False
        ...
        if platform == "partnersearch":
            pass
        elif platform not in REGISTRATION_URLS and not direct_company_card:
            ...
            # PUBDISC_REGISTRATION_SECOND_HOP_V1
            # Search-discovered listing sites often route "Add listing" ->
            # login first, and expose the real registration link only there.
            # A one-hop preflight used to fill the login form and miss legal
            # consent/SMS requirements on the actual signup page. Follow one
            # additional same-site registration link, GET/navigation only.
            if str(platform).startswith("pubdisc_"):
                ...
                for reg_form_sel in [
                    ...
                    try:
                        reg_form = page.locator(reg_form_sel).first
                        if reg_form.count() and reg_form.is_visible():
                            registration_form_visible = True
                            break
                    except Exception:
                        pass
                if not registration_form_visible:
                    for sel in [
                        'a[href*="/register" i]',
                        'a[href*="mode=register" i]',
                        'a[href*="action=register" i]',
        ...
        prefilled = {}
        try:
            prefilled = _prefill_account_fields(page, email, account)
        except Exception:
            prefilled = {}

        hard_checkpoint = _detect_hard_checkpoint(page)
        policy_checkpoint, policy_detail = _registration_policy_checkpoint(page, email, account, platform=platform)
        ...
        if not hard_checkpoint and not policy_checkpoint:
            sso_checkpoint, sso_detail = _external_sso_only_checkpoint(page)
            if sso_checkpoint:
                policy_checkpoint = sso_checkpoint
                policy_detail = sso_detail
        if not hard_checkpoint and platform == "pubdisc_orgpage_ru":
            # Orgpage does not expose a classic account-registration form here;
            # the first safe action is the company-card form itself. It requires
            # truthful legal/contact identity. Never fabricate OPF/address/phone.
            company_form = page.locator(
                'input[name="name"], textarea[name="shortDesc"], textarea[name="fullDesc"]'
            )
            if company_form.count():
                policy_checkpoint = "truthful_company_identity_required"
                policy_detail = (
                    "Orgpage company-card form is available, but publication requires "
                    "verified real company OPF/name/city/address/phone. BORIS will not "
                    "invent business identity; resume only when verified profile data exists."
                )
        ...
        elif checkpoint == "captcha_required" and policy_checkpoint == "age_declaration_required":
            checkpoint = "captcha_age_declaration_required"
            policy_detail = (
                "Площадка требует одним регистрационным шагом CAPTCHA и дату рождения/возраст. "
                "BORIS не решает CAPTCHA и не заполняет возрастные данные автоматически."
            )
        ...
        if not form_ready and not checkpoint:
            # Browser templates can hide the legal/CAPTCHA gate behind markup
            # that our generic field selectors do not recognise. Before
            # labelling that as an adapter failure, consult the read-only HTTP
            # classifier. Adopt only a fail-closed blocking checkpoint; never
            # promote a generic HTML <form> to safe auto-submit.
            fallback = _registration_http_fallback(platform, page.url or start_url)
            fallback_checkpoint = str((fallback or {}).get("checkpoint") or "")
            fallback_status = _registration_status_for_checkpoint(fallback_checkpoint) if fallback_checkpoint else ""
            if fallback_checkpoint and fallback_status in {"verification_required", "blocked", "warming"}:
                checkpoint = fallback_checkpoint
                policy_detail = str((fallback or {}).get("detail") or policy_detail or "")
            else:
                checkpoint = "registration_form_not_found"
        ...
        if submit and not checkpoint and form_ready:
            if platform == "forum_baza_1c":
                try:
                    ...
                    submitted = False
            elif platform == "joomlaforum_jobs":
                try:
                    # The page also has a navigation link named «Регистрация».
                    # Clicking by visible text can hit that link and falsely look
                    # like a successful submit while the account is never created.
                    form = page.locator('form:has(input[name="user"]):has(input[name="passwrd1"])').first
                    submit_button = form.locator('input[name="regSubmit"], button[type="submit"], input[type="submit"]').first
                ...
                try:
                    if "/register/complete" in page.url:
                        ...
                        if "/register/complete" in page.url:
                            submitted = False
                            checkpoint, result_detail = _registration_policy_checkpoint(page, email, account, platform)
                            if not checkpoint:
                                checkpoint = "terms_acceptance_required"
                                result_detail = (
                                    "Площадка открыла второй шаг регистрации с обязательным GDPR/согласием. "
                                    "BORIS не принимает такое согласие автоматически."
                                )
                except Exception:
                    submitted = False
            else:
    ...
    attempt_status = "blocked" if checkpoint in {
        ...
        "captcha_age_and_terms_required","captcha_age_declaration_required",
        "sms_or_verification_code_required","email_verification_required",
        "age_and_terms_declaration_required","age_declaration_required","terms_acceptance_required",
...
LOGIN_URLS = {
    ...
    "joomlaforum_jobs": "https://joomlaforum.ru/index.php?action=login",
...
POST_URLS = {
    ...
    "joomlaforum_jobs": "https://joomlaforum.ru/index.php?action=post;board=29.0",
...
def _semantic_checkpoint_error(checkpoint: str, previous_error: str | None) -> str:
    text = str(previous_error or "").strip()
    technical_markers = (
        "traceback", "playwright._impl", "call log:", "page.goto:",
        "locator.fill:", "locator.click:", "net::err_",
    )
    if text and not any(marker in text.lower() for marker in technical_markers):
        return text[:900]
    action = marketplace_svc.HUMAN_PLATFORM_BOOTSTRAP_CHECKPOINTS.get(checkpoint)
    if action:
        return f"{action}. BORIS сам перепроверит площадку после завершения этого одноразового шага."
    return "Одноразовая подготовка площадки ещё не подтверждена; BORIS перепроверит её автоматически."
    ...
    *,
    ...
    if (
        ...
        return (
            ...
            previous_checkpoint,
            _semantic_checkpoint_error(previous_checkpoint, previous_error or error),
        )
...
def _verify_timeout_state(previous_checkpoint: str, previous_error: str | None) -> tuple[str, str, str]:
    if previous_checkpoint in marketplace_svc.HUMAN_PLATFORM_BOOTSTRAP_CHECKPOINTS:
        return (
            "verification_required",
            previous_checkpoint,
            _semantic_checkpoint_error(previous_checkpoint, previous_error),
        )
    return (
        "in_progress",
        "login_probe_timeout",
...
def complete_human_checkpoint(platform: str, value: str, *, headless: bool = True) -> dict:
    """Submit a user-provided OTP/text challenge answer in the same registration flow.

    This intentionally refuses reCAPTCHA/hCaptcha/Turnstile token injection.
    The human must complete those interactive widgets in the browser.
    """
    value = str(value or "").strip()
    if not value:
        raise SystemExit("checkpoint value is required")
    ...
    with sync_playwright() as pw:
        ...
        # Follow ordinary registration links when the start URL is a forum home.
        if platform == "partnersearch":
            pass
        elif platform not in REGISTRATION_URLS:
            for sel in [
                'a[href*="/register" i]','a[href*="mode=register" i]',
                'a[href*="action=register" i]','a[href*="signup" i]',
            ]:
                try:
                    loc = page.locator(sel).first
                    if loc.count() and loc.is_visible():
        ...
        # Never inject tokens into interactive anti-bot widgets.
        html = ""
        try:
        ...
        if any(x in html for x in (
            ...
            "h-captcha-response","hcaptcha.com","challenges.cloudflare.com/turnstile",
        )):
            ctx.close()
            return {
                "ok": False,
                "platform": platform,
                "checkpoint": "interactive_captcha_required",
                "submitted": False,
                "message": "Интерактивную CAPTCHA нужно пройти вручную в браузере; токен BORIS не подделывает.",
            }

        filled = {}
        try:
            filled = _prefill_account_fields(page, email, account)
        except Exception:
            filled = {}

        # User-supplied OTP / text-CAPTCHA selectors only.
        answer_selectors = [
            'input[name="captcha_question_answer"]',
            'input[name="website_code"]',
        ...
        if target is None:
            ...
            return {
                "ok": False,
                "platform": platform,
                "checkpoint": "checkpoint_input_not_found",
                "submitted": False,
                "url": current,
            }
        ...
        checkpoint, detail = _registration_result(page)
        current_url = page.url
        try:
            body = page.locator("body").inner_text(timeout=4000)
        except Exception:
            body = ""
        success_hint = any(x in body.lower() for x in (
            "проверьте почту","подтвердите почту","verification email",
            "confirm your email","регистрация завершена","registration complete",
        ))
        ctx.close()
...
def verify_registration(platform: str, *, headless: bool = True) -> dict:
    ...
    with sync_playwright() as pw:
        ...
        try:
            ...
            if not isinstance(exc, PlaywrightTimeoutError) and not _is_transient_navigation_error(exc):
                raise
        ...
        if platform == "forum_baza_1c":
            ...
            if submitted:
                ...
                obvious_failure = any(x in first_body for x in [
                    ...
                    "неправильное имя или пароль","неверное имя или пароль",
...
def prepare_post(platform: str, draft_id: str, *, headless: bool, publish: bool) -> dict:
    ...
    requires_ready_account = bool(
        ...
        or platform.startswith("pubdisc_")
    ...
    # PRE_SUBMIT_CONTENT_LANDING_GATE_V1
    # Fail before opening or filling any external publication form. The
    # post-publication validator remains defense in depth only.
    content_gate = marketplace_svc.validate_publication_content(draft)
    if not content_gate.get("ok"):
        reason = str(content_gate.get("reason") or "content_landing_guard")
        marketplace_svc.record_attempt(
            platform=platform,
            action="prepare_post",
            ...
            error_code=reason,
            error_detail=(
                "Pre-submit SEO guard blocked the draft: landing/intent or the "
                "final 10-20 keyword phrase block violates Crowd SEO policy."
            ),
            rules_decision=str((audit or {}).get("decision") or ""),
            meta={"content_gate": content_gate},
        )
        raise SystemExit(f"publication blocked by content/landing guard: {reason}")
...
# PUBLICATION_BROWSER_PROOF_V1
# Read-only proof for an already-created topic. This intentionally reuses the
# platform's persisted browser profile and never clicks, submits forms, accepts
# terms or interacts with CAPTCHA. It exists only as a second verifier when a
# plain requests.get() probe is blocked by the site's HTTP/anti-bot layer.
def _publication_url_matches_platform(platform: str, publication_url: str) -> bool:
    actual = (urlparse(str(publication_url or "")).hostname or "").lower().removeprefix("www.")
    expected = (urlparse(_platform_start_url(platform)).hostname or "").lower().removeprefix("www.")
    if not actual or not expected:
        return False
    return actual == expected or actual.endswith("." + expected) or expected.endswith("." + actual)


def verify_publication(platform: str, publication_url: str, target_url: str, *, headless: bool = True) -> dict:
    publication_url = str(publication_url or "").strip()
    target_url = str(target_url or "").strip()
    if not publication_url or not target_url:
        return {"ok": False, "publication_verified": False, "error": "publication_or_target_url_missing"}
    if not _publication_url_matches_platform(platform, publication_url):
        return {"ok": False, "publication_verified": False, "error": "publication_url_platform_mismatch"}
    ...
    with sync_playwright() as p:
        ...
        page = ctx.pages[0] if ctx.pages else ctx.new_page()
        try:
            response = page.goto(publication_url, wait_until="domcontentloaded", timeout=45000)
            page.wait_for_timeout(900)
            proof = page.evaluate(
                """(target) => {
                    ...
                    const link = anchors.find((a) => {
                        ...
                        const abs = norm(a.href);
                        const text = norm((a.textContent || '').trim());
                        return raw === wanted || abs === wanted || text === wanted;
                    }) || null;
                    const robots = document.querySelector('meta[name="robots" i]');
                    const robotsContent = String(robots?.getAttribute('content') || '').toLowerCase();
                    return {
            ...
            final_url = str(page.url or publication_url)
            link_present = bool(proof.get("link_present"))
            indexable = status == 200 and "noindex" not in str(proof.get("robots") or "").lower()
            rel = str(proof.get("rel") or "").strip() or None
            verified = bool(link_present and indexable)
            return {
                "ok": True,
                "platform": platform,
                ...
                "rel": rel,
                "link_rel_type": (
                    "nofollow" if rel and "nofollow" in rel.lower() else ("dofollow" if link_present else None)
                ),
                "publication_verified": verified,
                "proof_source": "persisted_browser_session",
                "title": str(proof.get("title") or "")[:300],
                "error": None if verified else "target_link_missing_or_not_indexable",
            }
        except Exception as exc:
            return {
                "ok": False,
...
def main() -> int:
    ...
    hc = sub.add_parser("complete-checkpoint")
    hc.add_argument("--platform", required=True)
    hc.add_argument("--value", required=True)
    ...
    vp.add_argument("--platform", required=True)
    vp.add_argument("--publication-url", required=True)
    vp.add_argument("--target-url", required=True)
    vp.add_argument("--headed", action="store_true")
    ...
    elif args.cmd == "complete-checkpoint":
        out = complete_human_checkpoint(args.platform, args.value, headless=not args.headed)
    elif args.cmd == "verify-publication":
        out = verify_publication(
            args.platform,
