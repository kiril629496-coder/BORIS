...
class BootstrapVerifyApiTests(unittest.TestCase):
    def test_verify_bootstrap_runs_only_existing_queue_item_and_returns_ready(self):
        ...
        with (
            ...
            patch.object(api, "_resume_crowd_after_ready", return_value={"status": "completed"}) as resume,
        ):
        ...
        resume.assert_called_once_with("captcha_forum")
        argv = run.call_args.args[0]
        self.assertIn("verify-registration", argv)
        self.assertNotIn("--submit", argv)

    def test_complete_checkpoint_immediately_verifies_after_text_value_submit(self):
        proc = SimpleNamespace(
            returncode=0,
            stdout=json.dumps({"ok": True, "submitted": True, "checkpoint": "email_verification_required"}),
            stderr="",
        )
        verified = {
            "ok": True,
            "platform": "otp_forum",
            "ready": True,
            "queue": {"needed": 0, "items": []},
            "resume": {"status": "completed"},
        }
        with (
            patch.object(api.subprocess, "run", return_value=proc),
            patch.object(api, "verify_bootstrap", return_value=verified) as verify,
        ):
            result = api.complete_checkpoint("otp_forum", api.HumanCheckpointRequest(value="123456"))

        self.assertTrue(result["result"]["submitted"])
        self.assertTrue(result["verification"]["ready"])
        self.assertEqual(result["queue"]["needed"], 0)
        verify.assert_called_once_with("otp_forum")

    def test_resume_handoff_uses_guardian_systemd_no_block(self):
        proc = SimpleNamespace(returncode=0, stdout="", stderr="")
        with patch.object(api.subprocess, "run", return_value=proc) as run:
            result = api._resume_crowd_after_ready("ready_forum")
        self.assertEqual(result["status"], "scheduled")
        argv = run.call_args.args[0]
        self.assertEqual(argv[:3], ["systemctl", "start", "--no-block"])
        self.assertIn("boris-forum-acquisition-guard.service", argv)

    def test_verify_bootstrap_ready_item_outside_queue_still_schedules_resume(self):
        with (
            patch.object(api.svc, "platform_bootstrap_queue", return_value={"needed": 0, "items": []}),
            patch.object(api.svc, "list_platforms", return_value=[{"key": "ready_forum", "publication_ready": True, "registration_checkpoint": None}]),
            patch.object(api, "_resume_crowd_after_ready", return_value={"status": "scheduled"}) as resume,
        ):
            result = api.verify_bootstrap("ready_forum")
        self.assertTrue(result["ready"])
        self.assertEqual(result["resume"]["status"], "scheduled")
        resume.assert_called_once_with("ready_forum")
    ...
    def test_inventory_summary_counts_unique_sites_and_live_deficit(self):
        ...
        project = {"id": "p1", "site": "https://client.example/", "niche": "marketing", "keywords": [], "offer_type": "services", "plan_mode": "contract", "target_count": 18, "bonus_count": 0}
        ...
        self.assertEqual(result["sellable_product_formats"]["services"]["target"], 100)
        self.assertEqual(result["sellable_product_formats"]["services"]["active_order_target"], 18)
        self.assertEqual(result["sellable_product_formats"]["services"]["deficit"], 99)
        self.assertEqual(result["sellable_product_formats"]["goods"]["allowed_unique_sites"], 0)
        self.assertEqual(result["sellable_product_formats"]["goods"]["target"], 100)
        self.assertEqual(result["sellable_product_formats"]["goods"]["active_order_target"], 0)
        self.assertEqual(result["sellable_product_formats"]["goods"]["deficit"], 100)
        self.assertEqual(result["sellable_product_total_deficit"], 199)
    ...
    def test_inventory_sellable_excludes_terminal_registration_even_when_rule_allowed(self):
        ...
        self.assertEqual(result["sellable_product_formats"]["goods"]["target"], 100)
        self.assertEqual(result["sellable_product_formats"]["goods"]["active_order_target"], 0)
        self.assertEqual(result["sellable_product_formats"]["goods"]["deficit"], 99)
        self.assertFalse(result["sellable_product_formats"]["goods"]["sellable"])
