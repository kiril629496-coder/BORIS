...
def test_crowd_runtime_state_paths_follow_current_checkout():
    ...
    assert service_marketplace.ROOT == backend_root

    for tool_name in ("crowd_gateway_flow.py", "crowd_handoff_queue.py"):
        tool_source = (backend_root / "tools" / tool_name).read_text(encoding="utf-8")
        assert "/root/BORIS/backend" not in tool_source
        assert "Path(__file__).resolve().parents[1]" in tool_source
