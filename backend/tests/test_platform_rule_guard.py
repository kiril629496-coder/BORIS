...
class PlatformRuleGuardTests(unittest.TestCase):
    ...
    def test_saashub_rejects_custom_development_agency_inventory(self):
        platform = marketplace.Platform(
            "pubdisc_saashub_com", "SaaSHub", "https://www.saashub.com/services/submit",
            "direct_post", 1, "SEO", "submit product", "software directory",
        )
        inspection = {
            "url": "https://www.saashub.com/services/submit",
            "decision": "allowed",
            "requirements": [],
            "text_excerpt": "These types of products are not accepted: Software development agencies.",
            "outbound_links": [],
            "evidence": {},
            "http_status": 200,
        }
        with tempfile.TemporaryDirectory() as tmp:
            with patch.object(platform_rules.marketplace, "get_platform", return_value=platform), \
                 patch.object(platform_rules, "inspect_url", return_value=dict(inspection)), \
                 patch.object(platform_rules, "STATE_DIR", Path(tmp)), \
                 patch.object(platform_rules, "_publication_discovery_authoring_evidence", return_value={"found": True}):
                out = platform_rules.inspect_platform("pubdisc_saashub_com")
        self.assertEqual(out["decision"], "blocked")
        self.assertIn("software_development_agencies_not_accepted", out["requirements"])

    def test_gde_free_tier_is_blocked_when_mandatory_site_link_is_paid(self):
        platform = marketplace.Platform(
            "pubdisc_gde_ru", "Gde", "https://gde.ru/post",
            "direct_post", 1, "SEO", "free listing", "classified",
        )
        inspection = {
            "url": "https://gde.ru/post",
            "decision": "allowed",
            "requirements": [],
            "text_excerpt": "Ссылка на сайт (платно). Не указывайте контактные данные в заголовке и тексте объявления.",
            "outbound_links": [],
            "evidence": {},
            "http_status": 200,
        }
        with tempfile.TemporaryDirectory() as tmp:
            with patch.object(platform_rules.marketplace, "get_platform", return_value=platform), \
                 ...
                 patch.object(platform_rules, "STATE_DIR", Path(tmp)), \
                 patch.object(platform_rules, "_publication_discovery_authoring_evidence", return_value={"found": True}):
                out = platform_rules.inspect_platform("pubdisc_gde_ru")
        self.assertEqual(out["decision"], "blocked")
        self.assertIn("mandatory_boris_link_incompatible_with_free_tier", out["requirements"])
