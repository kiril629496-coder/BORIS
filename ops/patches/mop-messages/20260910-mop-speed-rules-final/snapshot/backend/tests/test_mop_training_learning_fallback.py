...
def test_general_rule_with_quoted_examples_is_never_executable():
    rules = [{
        "rule": (
            "Правило для всех МОПов: короткие ответы клиента «да», «нет», «нету» "
            "тоже считаются ответом."
        )
    }]
    assert messenger._mop_deterministic_learning_reply(
        "интересует аренда авто с выкупом", rules
    ) is None


def test_short_trigger_does_not_match_inside_longer_word():
    rules = [{
        "rule": "Если клиент пишет «да», ответь ровно «Принято»."
    }]
    assert messenger._mop_deterministic_learning_reply(
        "интересует аренда авто", rules
    ) is None
