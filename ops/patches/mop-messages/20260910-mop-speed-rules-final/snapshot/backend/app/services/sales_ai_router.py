...
def provider_order(account_id: str | None = None) -> list[str]:
    ...
    if (
        ...
        and str(os.getenv("PLUSVIBE_API_KEY") or os.getenv("DEEPSEEK_API_KEY") or "").strip()
...
def _deepseek_default_call(
    *,
    ...
    key = str(os.getenv("PLUSVIBE_API_KEY") or os.getenv("DEEPSEEK_API_KEY") or "").strip()
    if not key:
        raise SalesAIProviderUnavailable("deepseek/plusvibe key missing")
    ...
    is_plusvibe = "plusvibeapi.ru" in base.lower()
    body = {
        "model": str(model),
        "messages": messages,
        "max_tokens": 16000,
    ...
    # Native DeepSeek accepts its `thinking` extension. PlusVibe exposes an
    # OpenAI-compatible contract, where reasoning is selected with
    # `reasoning_effort` and unknown provider-specific fields should be omitted.
    if not is_plusvibe:
        body["thinking"] = {"type": "disabled" if fast_non_thinking else "enabled"}
    if fast_non_thinking:
        body["temperature"] = float(os.getenv("BORIS_DEEPSEEK_MOP_TEMPERATURE", "0.15") or 0.15)
    elif not (is_plusvibe and str(model).lower().startswith("minimax-")):
...
def generate_text(
    *,
    ...
    for _idx, provider in enumerate(order):
        ...
        # MOP_OLLAMA_PRESTART_PRESSURE_FAST_FAIL_V1:
        # A remote provider may have just consumed part of the interactive
        # budget. Before spending the remaining client wait on Ollama, perform
        # the same cheap production-pressure check used by the local adapter.
        # If current pressure already proves local inference unsafe, skip it
        # immediately and let messenger return its deterministic continuation.
        if (
            str(module or "").strip().lower() == "mop"
            and provider == "ollama"
        ):
            try:
                from app.ext_api import sched as _mop_sched
                _mop_pressure = _mop_sched.pressure()
            except Exception as _mop_pressure_exc:
                chain.append({
                    "provider": "ollama",
                    "reason": "local_pressure_check_failed",
                    "detail": type(_mop_pressure_exc).__name__,
                })
                continue
            _mop_historical_headroom = bool(
                _mop_pressure.get("under_pressure")
                and _mop_pressure.get("historical_load_only")
                and _mop_pressure.get("instant_headroom")
            )
            if _mop_pressure.get("under_pressure") and not _mop_historical_headroom:
                chain.append({
                    "provider": "ollama",
                    "reason": "local_pressure_fast_fail",
                    "detail": ", ".join(_mop_pressure.get("reasons") or [])[:220],
                })
                continue
