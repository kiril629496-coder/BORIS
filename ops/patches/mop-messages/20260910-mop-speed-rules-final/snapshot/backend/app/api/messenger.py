...
# AVITO_SYSTEM_EMPTY_CHAT_TRIGGER_V1:
# MOP_SYSTEM_PROACTIVE_NUDGE_ROUTER_V1:
# Avito emits a few business-significant events as inbound *system* messages.
# Only explicitly classified events may wake the proactive MOP. All other
# system messages remain fail-closed and never become ordinary dialogue.
def _system_proactive_event(text_value: str) -> str | None:
    normalized = " ".join(str(text_value or "").replace(" ", " ").split()).lower()
    if not normalized.startswith(_SYSTEM_NUDGE_PREFIX.lower()):
        return None
    if "создал чат" in normalized and "пока ничего не написал" in normalized:
        return "empty_chat"
    if "посмотрел номер из объявления" in normalized:
        return "viewed_phone"
    return None


def _is_system_empty_chat_nudge(text_value: str) -> bool:
    return _system_proactive_event(text_value) == "empty_chat"


def _is_system_viewed_phone_nudge(text_value: str) -> bool:
    return _system_proactive_event(text_value) == "viewed_phone"
...
def _mop_effective_learning_rules(rules: list[dict] | None) -> list[dict]:
    """Return de-duplicated owner training that may safely reach live MOP replies.

    Historical feedback stays stored for audit, but training cannot override
    non-negotiable client-facing identity / no-false-promise constraints.
    """
    import re as _elr_re
    out: list[dict] = []
    seen: set[str] = set()
    blocked_markers = (
        "меня зовут борис",
        "я помощник марии",
        "я помощник платформы",
        "я бот",
        "я ии",
        "я искусственный интеллект",
        "наш эксперт ответит на все вопросы",
        "максимально подробно разберем вашу ситуацию",
    )
    for item in rules or []:
        ...
        low = " ".join(
            _elr_re.findall(
                r"[а-яёa-z0-9]+",
                rule.lower().replace("ё", "е"),
                _elr_re.I,
            )
        )
        if not low or any(marker in low for marker in blocked_markers):
            continue
        if low in seen:
            continue
        seen.add(low)
...
def _mop_deterministic_learning_reply(question: str, rules: list[dict] | None) -> str | None:
    ...
    for item in rules or []:
        ...
        # Execute only an explicit owner command. General prose that merely
        # quotes examples ("да", "нет", "нету") must never become an auto-reply.
        explicit = _dlr_re.search(
            r"\bесли\s+клиент\b.{0,80}?"
            r"(?:пишет|написал|написала|говорит|сказал|сказала|спрашивает)\s*"
            r'["«“](.+?)[»”"]'
            r".{0,160}?\bответ(?:ь|ить|итьте|ьте)\w*\b(?:\s+ровно)?\s*"
            r'["«“](.+?)[»”"]',
            rule,
            _dlr_re.I,
        )
        if not explicit:
            continue
        trigger = _norm(explicit.group(1))
        reply = str(explicit.group(2) or "").strip()
        if trigger and reply and _contains_token_phrase(q_norm, trigger):
            return reply[:1000]
    return None


def _mop_high_confidence_first_turn_reply(question: str, history_text: str) -> str | None:
    """Zero-AI reply for a few unambiguous first-turn sales intents.

    This is deliberately narrow: it never names commercial facts and never
    overrides confirmed owner training. Its purpose is to keep common first
    replies fast even while remote/local AI is unavailable or CPU-fenced.
    """
    import re as _hf_re
    q = str(question or "").strip()
    if not q:
        return None
    client_lines = _history_role_messages(history_text, "client")
    our_lines = _history_role_messages(history_text, "ours")
    if len(client_lines) != 1 or our_lines:
        return None
    low = q.lower().replace("ё", "е")
    if _hf_re.search(
        r"(?:хочу|нужно|нужна|ищу|интересует).{0,35}(?:подобрат|подбор).{0,35}(?:квартир|недвиж)|"
        r"(?:подобрат|подбор).{0,35}(?:квартир|недвиж)|"
        r"(?:хочу|нужно|ищу).{0,25}(?:квартир|недвиж).{0,25}(?:москв|район|метро)",
        low,
    ):
        return "Начнём с основных параметров. Какой бюджет рассматриваете?"
    if _hf_re.search(r"(?:интересует|нужна|хочу).{0,25}рефинанс", low):
        return "Рефинансировать хотите ипотеку или другие кредиты?"
    if _hf_re.search(r"(?:интересует|нужна|хочу).{0,25}ипотек", low):
        return "Ипотека нужна уже под выбранную квартиру или сначала хотите получить предварительное одобрение?"
    return None
...
def _initial_short_subject_clarification(question: str, history_text: str) -> str | None:
    ...
    # MOP_SHORT_INTEREST_FIRST_TURN_V1:
    # A greeting plus a short generic interest ("добрый день интересует ипотека")
    # is still a clarification turn, not a reason to wait for remote AI.
    # Keep commercial/specific questions on the normal guards/router.
    _specific_markers = (
        "цена", "стоим", "сколь", "процент", "ставк", "услов", "срок",
        "документ", "одобр", "первонач", "ежемесяч", "платеж", "расчет",
        "рассч", "рефинанс", "можно ли", "как ", "почему", "когда",
    )
    _short_interest = bool(_short_re.search(
        r"^(?:(?:здравствуйте|привет|добрый\s+(?:день|вечер)|доброе\s+утро)[,! ]*)?"
        r"(?:(?:меня\s+)?интересует|нужна\s+консультация\s+по|хочу\s+узнать\s+про)\s+.+$",
        low,
        _short_re.I,
    ))
    if _short_interest and not any(x in low for x in _specific_markers):
        return "Добрый день! Понял ваш запрос. Подскажите, пожалуйста, что именно хотите уточнить?"

    # HUMAN_FIRST_TURN_SENTENCE_GUARD_V1:
    # This zero-AI fast path is only for a bare subject ("Ипотека",
    # "Ремонт фар"), not for a real sentence.
    sentence_markers = (
...
def _history_role_blocks(history_text: str) -> list[dict]:
    """Preserve chronological role blocks for deterministic continuity guards."""
    blocks = []
    current_role = None
    buf = []
    ...
    def flush():
        ...
        if current_role and buf:
            value = "\n".join(x for x in buf if x).strip()
            if value:
                blocks.append({"role": current_role, "text": value})
        buf = []
...
def _mop_question_signature(text_value: str) -> set[str]:
    """Normalize a customer-facing question into a compact semantic signature."""
    import re as _qs_re
    low = str(text_value or "").lower().replace("ё", "е")
    stop = {
        "подскажите","пожалуйста","скажите","уточните","есть","ли","какой","какая",
        "какие","какое","как","что","чем","у","вас","вам","для","по","в","на","и",
        "или","нужен","нужна","нужно","нужны","можно","примерно","примерные",
    }
    words = _qs_re.findall(r"[а-яa-z0-9]+", low, _qs_re.I)
    sig: set[str] = set()
    for word in words:
        if word in stop or len(word) <= 2:
            continue
        sig.add(word[:6] if len(word) > 6 else word)
    return sig


def _mop_client_response_closes_question(text_value: str) -> bool:
    """A real answer closes a question, including «нет», «да» and «не знаю»."""
    import re as _qr_re
    low = " ".join(str(text_value or "").lower().replace("ё", "е").split())
    if not low:
        return False
    if _qr_re.fullmatch(r"(?:что|а|эм|мм|\?+)", low):
        return False
    if _qr_re.search(
        r"(?:не\s+понял|не\s+поняла|непонят|повтор(?:и|ите)|еще\s+раз|"
        r"в\s+смысле|что\s+имеете\s+в\s+виду|поясните|уточните\s+вопрос)",
        low,
        _qr_re.I,
    ):
        return False
...
def _mop_question_was_answered(history_text: str, candidate_question: str) -> bool:
    """Detect a same/near-same question that already got a client response."""
    blocks = _history_role_blocks(history_text)
    target = _mop_question_signature(candidate_question)
    if not target:
        return False
    for idx, block in enumerate(blocks):
        if block.get("role") != "ours":
            continue
        asked = str(block.get("text") or "").strip()
        if "?" not in asked:
            continue
        sig = _mop_question_signature(asked)
        if not sig:
            continue
        inter = len(target & sig)
        union = max(1, len(target | sig))
        similarity = inter / union
        same = bool(
            target == sig
            or similarity >= 0.72
            or (len(target) >= 2 and target.issubset(sig))
            or (len(sig) >= 2 and sig.issubset(target))
        )
        if not same:
            continue
        for later in blocks[idx + 1:]:
            if later.get("role") == "ours":
                break
            if later.get("role") == "client":
                return _mop_client_response_closes_question(later.get("text") or "")
    return False


def _mop_reply_repeats_answered_question(reply_text: str, history_text: str) -> bool:
    """Catch repeated semantic questions outside the hard-coded slot list."""
    import re as _rq_re
    reply = str(reply_text or "")
    questions = [q.strip() + "?" for q in _rq_re.findall(r"([^?]{3,})\?", reply)]
    return any(_mop_question_was_answered(history_text, q) for q in questions)


def _mop_token_distance(a: str, b: str) -> int:
    """Small deterministic Levenshtein distance for typo-tolerant intent cues."""
    a = str(a or "")
    b = str(b or "")
    if a == b:
        return 0
...
def _mop_measurement_intent(text_value: str) -> bool:
    """Recognize measurement/measurer intent, including ordinary customer typos."""
    import re as _mi_re
    low = str(text_value or "").lower().replace("ё", "е")
    if _mi_re.search(
        r"\b(?:замер\w*|мерк\w*|обмер\w*)\b",
        low,
        _mi_re.I,
    ):
        return True
    tokens = _mi_re.findall(r"[а-я]{5,14}", low, _mi_re.I)
    targets = ("замерщик", "замерить", "замерщикa", "замерщика")
    for token in tokens:
        for target in targets:
            limit = 1 if max(len(token), len(target)) <= 7 else 2
            if abs(len(token) - len(target)) <= limit and _mop_token_distance(token, target) <= limit:
                return True
    return bool(_mi_re.search(
        r"(?:не\s+зна\w*.{0,35}(?:размер|габарит)|(?:размер|габарит)\w*.{0,25}(?:нет|не\s+зна))",
        low,
        _mi_re.I,
    ))


def _mop_dimensions_answered(text_value: str) -> bool:
    """Dimensions are answered either by actual dimensions or by explicit need for measurement."""
    import re as _md_re
    low = str(text_value or "").lower().replace("ё", "е")
    return bool(
        _md_re.search(
            r"\d{1,4}(?:[.,]\d+)?\s*(?:[xх×]\s*\d{1,4}(?:[.,]\d+)?|м\s*(?:2|²)|кв\.?\s*м)",
            low,
            _md_re.I,
        )
        or _mop_measurement_intent(low)
    )


def _mop_measurement_repair_reply(history_text: str) -> str:
    """Safe continuation after client says dimensions are unknown / a measurer is needed."""
    import re as _mr_re
    client_history = "\n".join(_history_role_messages(history_text, "client")).lower().replace("ё", "е")
    has_address = bool(_mr_re.search(
        r"\b(?:ул\.?|улиц\w*|проспект\w*|пр\.?|шоссе|д\.?\s*\d|дом\s*\d)\b",
        client_history,
        _mr_re.I,
    ))
    if has_address:
        return "Понял, нужен замерщик. Размеры повторно спрашивать не буду. Подскажите, когда вам удобнее провести замер?"
    return "Понял, нужен замерщик. Размеры повторно спрашивать не буду. Подскажите адрес объекта, куда нужен замер?"


# MOP_PHONE_10_15_DIGIT_CONTACT_V1:
# One canonical contact detector for live MOP, training, qualification and policy guards.
def _mop_text_has_phone(text_value: str) -> bool:
    """High-precision phone detector for client text.

    Accept Russian formats plus ordinary international/contact numbers with
    10-15 digits (E.164 maximum is 15). This intentionally excludes 16-digit
    bank-card-like values and short order/price numbers.
    """
    ...
    if bool(
        ...
        return True
    for candidate in _phone_re.findall(
        r"(?<!\w)\+?\d(?:[\d\s()\-]{8,28}\d)?(?!\w)",
        raw,
    ):
        digits=_phone_re.sub(r"\D","",candidate)
        if 10 <= len(digits) <= 15:
            return True
    return False


# MOP_PHONE_CONTEXT_MEMORY_V1:
# A contact can be known from the dialogue even when the client does not repeat
# the digits in the current message ("телефон уже оставила в заявке").
def _mop_phone_known(text_value: str) -> bool:
    """True only when the dialogue contains positive evidence that a phone/contact exists."""
    import re as _known_re
    raw=str(text_value or "")
    if _mop_text_has_phone(raw):
        return True
    low=" ".join(raw.lower().replace("ё","е").split())
    if not low:
        return False

    # Explicit refusal / not-yet-given evidence wins over words like "оставлял".
    negative=bool(
        _known_re.search(
            r"(?:номер|телефон|контакт).{0,45}\b(?:пока\s+не|еще\s+не|не)\s+"
            r"(?:остав\w*|да(?:л|ла|вали?)\w*|отправ\w*|пис\w*|указ\w*|присыл\w*)",
            low,
        )
        or _known_re.search(
            r"\b(?:не\s+хочу|не\s+буду|не\s+готов\w*|отказываюсь)\b.{0,45}"
            r"(?:номер|телефон|контакт)",
            low,
        )
    )
    if negative:
    ...
    return bool(
        _known_re.search(
            r"(?:номер|телефон|контакт).{0,80}\b(?:есть|остав\w*|да(?:л|ла|ли)\b|"
            r"отправ\w*|пис\w*|указ\w*|присла\w*|внес\w*|заполн\w*)",
            low,
        )
        or _known_re.search(
            r"\b(?:остав\w*|да(?:л|ла|ли)\b|отправ\w*|пис\w*|указ\w*|"
            r"присла\w*|внес\w*).{0,50}(?:номер|телефон|контакт)",
            low,
        )
        or _known_re.search(
            r"(?:номер|телефон|контакт).{0,40}\b(?:в|у)\s+"
            r"(?:заявк\w*|анкет\w*|профил\w*|объявлен\w*)",
            low,
        )
    )


def _mop_first_manager_turn(db, account_id: str, chat_id: str, chat: dict | None = None) -> bool:
    """Return True only when no company/MOP reply exists in this dialogue yet."""
    try:
        training=(chat or {}).get("_training_messages") if isinstance(chat,dict) else None
        if isinstance(training,list):
            return not any(
                str(m.get("role") or "").lower() in {"mop","ours","seller","manager"}
                for m in training
            )
        if not chat_id:
            return False
        from sqlalchemy import text as _fmt_sql
        count=db.execute(_fmt_sql("""
            SELECT COUNT(*) FROM messenger_messages
             WHERE account_id=:a AND avito_chat_id=:c
               AND lower(coalesce(direction,'')) LIKE 'out%'
        """),{"a":str(account_id or ""),"c":str(chat_id or "")}).scalar() or 0
        return int(count)==0
    except Exception:
        return False
...
def _deterministic_mop_qualification(history_text: str, reply_text: str, analysis: dict | None = None) -> dict:
    ...
    phone_received = _mop_phone_known(client_text)
...
def _mop_client_meta_nonanswer(text_value: str) -> bool:
    """Client says 'I already wrote it' instead of answering the current question."""
    import re as _meta_re
    low = " ".join(str(text_value or "").lower().replace("ё", "е").split())
    if not low:
        return False
    return bool(_meta_re.fullmatch(
        r"(?:я\s+)?(?:же\s+)?(?:уже\s+)?(?:написал|написала|писал|писала|указал|указала)"
        r"(?:\s+(?:выше|это|вам))?|(?:выше|ранее)\s+(?:написал|написала|писал|писала|указал|указала)"
        r"|я\s*написал|я\s*написала",
        low,
        _meta_re.I,
    ))


def _mop_meta_nonanswer_reply(history_text: str) -> str:
    """Re-ask the unresolved last question without jumping to a new stage."""
    ours = _history_role_messages(history_text, "ours")
    last_q = (ours[-1] if ours else "").strip()
    low = last_q.lower().replace("ё", "е")
    branch = _mop_sales_branch(history_text)
    if branch == "mortgage" and (
        "предварительн" in low or "выбран" in low or "под выбран" in low
    ):
        return "Да, вижу — вам нужна ипотека. Уточню только один момент: квартира уже выбрана или сначала нужно предварительное одобрение?"
    if branch == "refinance" and "рефинанс" in low:
        return "Да, вижу. Уточню только: рефинансировать хотите ипотеку или другие кредиты?"
    if branch == "collateral" and ("недвижим" in low or "залог" in low):
        return "Да, вижу. Уточню только: какую недвижимость планируете использовать в залог?"
    if "?" in last_q:
        # Keep only the actual final question sentence, not a long company intro.
        import re as _q_re
        parts = _q_re.findall(r"[^.!?]*\?", last_q)
        q = (parts[-1] if parts else last_q).strip()
        if q:
            return "Да, вижу. Уточню только этот момент: " + q[0].lower() + q[1:]
    return _mop_branch_next_question(history_text) or "Да, вижу ваш предыдущий ответ. Уточните, пожалуйста, последний вопрос."


def _mop_sales_branch(history_text: str) -> str:
    """Detect current sales branch from client messages only."""
    import re as _branch_re
    client_text = " ".join(_history_role_messages(history_text, "client")).lower().replace("ё", "е")
    if not client_text:
        return ""
    if _branch_re.search(r"\bрефинанс\w*\b|консолидац\w*\s+кредит", client_text):
        return "refinance"
    if _branch_re.search(r"\bкредит\w*.{0,25}\bпод\s+залог\b|\bпод\s+залог\w*.{0,25}\bнедвиж", client_text):
        return "collateral"
    if _branch_re.search(r"\bипотек\w*\b", client_text):
        return "mortgage"
    if _branch_re.search(
        r"(?:подобрат|подбор|ищу|нужн\w*).{0,35}(?:квартир|недвиж)|"
        r"(?:квартир|недвиж).{0,35}(?:подобрат|подбор|ищу)",
        client_text,
    ):
...
def _mop_branch_next_question(history_text: str) -> str:
    """Intent-aware next question for common high-value branches."""
    import re as _bq_re
    branch = _mop_sales_branch(history_text)
    if not branch:
        return ""

    client_messages = _history_role_messages(history_text, "client")
    our_messages = _history_role_messages(history_text, "ours")
    client_text = " ".join(client_messages).lower().replace("ё", "е")
    our_text = " ".join(our_messages).lower().replace("ё", "е")
    last_client = (client_messages[-1] if client_messages else "").strip().lower().replace("ё", "е")
    last_ours = (our_messages[-1] if our_messages else "").strip().lower().replace("ё", "е")

    def asked(pattern: str) -> bool:
        return bool(_bq_re.search(pattern, our_text, _bq_re.I))

    def known(pattern: str) -> bool:
        return bool(_bq_re.search(pattern, client_text, _bq_re.I))

    def has_money() -> bool:
        return bool(_bq_re.search(
            r"(?<!\d)\d{1,3}(?:[\s.,]\d{3})*(?:[.,]\d+)?\s*(?:млн|миллион\w*|тыс\w*|руб\w*|₽)",
            client_text,
            _bq_re.I,
        ))

    if branch == "mortgage":
        property_context = bool(_bq_re.search(
            r"\b(?:новострой\w*|вторич\w*|квартир\w*|студи\w*|"
            r"однуш\w*|двуш\w*|тр[её]ш\w*|[1-6][ -]?(?:комн|кк)\w*)\b",
            client_text,
            _bq_re.I,
        ))
        if not property_context and not asked(r"(?:предварительн\w*\s+одобр|под\s+выбранн\w*\s+квартир|на\s+покупк\w*)"):
            return "Ипотека нужна уже под выбранную квартиру или сначала хотите получить предварительное одобрение?"
        if not known(r"\b(?:новострой\w*|вторич\w*)\b") and not asked(r"(?:новострой|вторич)"):
            return "Рассматриваете новостройку или вторичный рынок?"
        if not has_money() and not asked(r"(?:бюджет|стоимост\w*\s+покупк|сумм\w*\s+ипотек)"):
            room_ack = ""
            if "комнат" in last_ours:
                room_names = {
                    "1": "однокомнатная",
                    "2": "двухкомнатная",
                    "3": "трёхкомнатная",
                ...
                key = last_client.strip(" .,!").split()[0] if last_client else ""
                if key in room_names:
                    room_ack = "Понял, " + room_names[key] + ". "
            return room_ack + "Какой примерно бюджет покупки рассматриваете?"
        if not known(r"(?:первоначальн\w*\s+взнос|взнос\w*).{0,25}(?:\d|нет|без)") and not asked(r"(?:первоначальн\w*\s+взнос|какой\s+взнос)"):
            return "Какой первоначальный взнос планируете?"
        if not known(r"(?:официальн\w*|самозанят\w*|\bип\b|по\s+найму|трудоустро\w*|доход\w*)") and not asked(r"(?:доход|трудоустро|официальн)"):
            return "Есть подтверждаемый доход или нужна программа с альтернативным подтверждением?"
        if not known(r"(?:действующ\w*\s+кредит|кредитн\w*\s+карт|кредитов\s+нет|нет\s+кредит)") and not asked(r"(?:действующ\w*\s+кредит|есть\s+кредит)"):
            return "Есть сейчас действующие кредиты?"
        if not _mop_phone_known(client_text):
            return "Если удобно, оставьте номер телефона — специалист сможет быстро уточнить варианты по вашей ситуации."
        if not known(r"(?:сегодня|завтра|недел\w*|месяц\w*|в\s+ближайш\w*|планир\w*.{0,20}сделк)") and not asked(r"(?:когда\s+планиру|срок\w*.{0,20}сделк)"):
            return "Когда примерно планируете выходить на сделку?"
        return ""

    if branch == "refinance":
        if not asked(r"(?:ипотек|друг\w*\s+кредит).{0,20}(?:рефинанс|объедин)"):
            return "Рефинансировать хотите ипотеку или другие кредиты?"
        if not has_money() and not asked(r"(?:остаток|сумм\w*\s+долг|сколько\s+остал)"):
            return "Какая примерно общая сумма остатка по кредитам?"
        if not asked(r"(?:снизить\s+платеж|снизить\s+ставк|объединить)"):
            return "Что для вас важнее: снизить ежемесячный платёж, ставку или объединить несколько кредитов?"
        if not _mop_phone_known(client_text):
            return "Если удобно, оставьте номер телефона — специалист уточнит, какие варианты рефинансирования подходят."
        if not asked(r"(?:когда\s+удобно|время\s+для\s+созвон)"):
            return "Когда удобно обсудить детали со специалистом?"
        return ""

    if branch == "collateral":
        if not known(r"\b(?:квартир|дом|апартамент|коммерческ\w*|земл\w*|недвижимост)\b") and not asked(r"(?:какая\s+недвижимост|что\s+за\s+недвижимост)"):
            return "Какую недвижимость планируете использовать в залог?"
        if not known(r"\b(?:москва|московск\w*\s+област|\bмо\b|санкт[ -]?петербург|спб|област\w*|город\w*)\b") and not asked(r"(?:где\s+находится|город|регион)"):
            return "В каком городе или регионе находится недвижимость?"
        if not has_money() and not asked(r"(?:сумм\w*\s+нужн|какая\s+сумм)"):
            return "Какая сумма кредита вам нужна примерно?"
        if not _mop_phone_known(client_text):
            return "Если удобно, оставьте номер телефона — специалист уточнит возможные условия по объекту."
        if not asked(r"(?:когда\s+удобно|время\s+для\s+созвон)"):
            return "Когда удобно обсудить объект и сумму со специалистом?"
        return ""

    return ""


def _mop_emergency_next_question(required_questions, history_text: str, account_id: str = "") -> str:
    ...
    import re as _eq_re
    branch = _mop_sales_branch(history_text)
    if branch in {"mortgage", "refinance", "collateral"}:
        return _mop_branch_next_question(history_text)
    ...
    def _known(question: str) -> bool:
        # QUESTION_MEMORY_V2: once this semantic question was asked and the
        # client answered it, never ask it again unless the client explicitly
        # requested repetition/clarification.
        if _mop_question_was_answered(history_text, question):
            return True
        q = question.lower().replace("ё", "е")
        if "телефон" in q or "номер" in q or "контакт" in q:
            return bool(
                _mop_phone_known(client_text)
                or _eq_re.search(r"(?:телефон|номер|контакт).{0,35}(?:уже\s+)?(?:оставил|оставила|оставлял|оставляла|отправил|отправила|писал|писала|дал|дала)", client_text)
            )
        if "мягко выяснить задачу" in q or "выяснить задачу клиента" in q:
            # A substantive customer message already *is* the task. Do not
            # replace a clear request with the robotic «какая у вас задача?».
            clean = _eq_re.sub(r"\b(?:здравствуйте|привет|добрый\s+(?:день|вечер|утро)|спасибо)\b", " ", client_text)
            return len(_eq_re.findall(r"[а-яa-z0-9]+", clean, _eq_re.I)) >= 4
        if "предложить короткий созвон" in q or "предложить созвон" in q:
            # This is a behavior instruction, not a qualification slot. Never
            # render it literally as a question to the customer. If contact is
            # missing, the dedicated phone requirement below owns that question.
            return True
        ...
        if "размер" in q or "площад" in q:
            return _mop_dimensions_answered(client_text)
        # REAL_ESTATE_QUALIFICATION_MEMORY_V1:
        # City is not the same thing as district/metro. For free-form district
        # answers (e.g. "Печатники") the generic asked->answered memory above
        # is the canonical signal after we ask the question once.
        if "район" in q or "метро" in q:
            return False
        if "город" in q or "где находится" in q:
        ...
        if "бюджет" in q or "сумм" in q:
            return bool(_eq_re.search(
                r"(?<!\d)\d{1,3}(?:[\s.,]\d{3})*(?:[.,]\d+)?\s*(?:млн|миллион\w*|тыс\w*|руб\w*)",
                client_text,
            ))
        if "новострой" in q or "вторич" in q:
            return bool(_eq_re.search(r"\b(?:новострой\w*|вторич\w*)\b", client_text))
        if "комнат" in q:
            return bool(_eq_re.search(
                r"\b(?:студи\w*|однуш\w*|двуш\w*|тр[её]ш\w*|одно(?:комнат)\w*|двух(?:комнат)\w*|тр[её]х(?:комнат)\w*|четыр[её]х(?:комнат)\w*|[1-6][ -]?(?:комн|кк)\w*)\b",
                client_text,
            ))
        if "наличн" in q or "ипотек" in q or "способ оплат" in q:
            return bool(_eq_re.search(
                r"\b(?:наличн\w*|ипотек\w*|семейн\w*\s+ипотек\w*)\b",
                client_text,
            ))
        if "тип мебели" in q:
    ...
    def _humanize(question: str) -> str:
        ...
        if low in {"район или метро", "район/метро"}:
            return "Какой район или метро рассматриваете?"
        if low in {"новостройка или вторичный рынок", "новостройка/вторичный рынок"}:
            return "Рассматриваете новостройку или вторичный рынок?"
        if low in {"комнатность", "сколько комнат"}:
            return "Сколько комнат рассматриваете?"
        if low == "бюджет":
            return "Какой бюджет рассматриваете?"
        if low in {"наличные или ипотека", "способ оплаты: наличные или ипотека"}:
            return "Покупка планируется за наличные или с ипотекой?"
...
def _mop_account_next_question(account_id: str, history_text: str) -> str:
    """Return a non-dead-end next step using the account's own sales settings."""
    account_id = str(account_id or "").strip()
    try:
        db = SessionLocal()
        try:
            cfg = _mop_sales_settings_cfg(db, account_id) if account_id else {}
            row = db.execute(
                text("SELECT client_goal,client_goal_text FROM accounts WHERE account_id=:a"),
                {"a": account_id},
    ...
    q = _mop_emergency_next_question(
        (cfg or {}).get("required_questions") or [],
        history_text,
    ...
    try:
        from app.services.mop_dialogue_brain import next_learned_question
        learned_q = next_learned_question(account_id, history_text)
    except Exception:
        learned_q = ""
    if learned_q:
        return learned_q

    client_text = " ".join(_history_role_messages(history_text, "client"))
    goal_text = " ".join(str(x or "") for x in (row or ())).lower().replace("ё", "е")
    if not _mop_phone_known(client_text) and any(x in goal_text for x in ("телефон", "контакт", "созвон")):
        return "Если удобно, оставьте номер телефона для связи — специалист продолжит по вашему запросу."
    if _mop_phone_known(client_text):
        return "Основные данные уже зафиксированы. Когда вам удобно перейти к следующему шагу?"
    return "Основные данные уже зафиксированы. Какой следующий шаг для вас удобнее: продолжить здесь или перейти к короткому созвону?"
...
def _mop_echo_repair_reply(history_text: str) -> str:
    """Deterministic useful continuation when a small/local model echoes client text."""
    import re as _echo_re
    client_lines = _history_role_messages(history_text, "client")
    last = (client_lines[-1] if client_lines else "").strip()
    low = last.lower().replace("ё", "е")
    if _echo_re.search(r"(?:подобрат|подбор).{0,35}(?:квартир|недвиж)|(?:квартир|недвиж).{0,35}(?:подобрат|подбор)", low):
        return "Начнём с основных параметров. Какой бюджет рассматриваете?"
    if _echo_re.search(r"рефинанс", low):
        return "Рефинансировать хотите ипотеку или другие кредиты?"
    if _echo_re.search(r"ипотек", low):
        return "Ипотека нужна уже под выбранную квартиру или сначала хотите получить предварительное одобрение?"
    if _echo_re.search(r"кредит.{0,20}залог", low):
        return "Какую недвижимость планируете использовать в залог?"
    if _echo_re.search(r"(?:цена|стоимост|сколько\s+стоит)", low):
        return "Подскажите, пожалуйста, о каком именно объекте или услуге идёт речь?"
    return "Понял ваш запрос. Подскажите, пожалуйста, какой результат для вас сейчас главное получить?"
...
def _mop_output_policy_violations(reply_text: str, history_text: str = "", trusted_context: str = "") -> list[str]:
    ...
    # MOP_AUTOMATION_DISCLOSURE_GUARD_V1:
    # The customer must experience a normal human sales conversation. Never
    # explain that a previous/current answer was a mailing, auto-reply, bot,
    # neural network or AI-generated message.
    if (
        _policy_re.search(
            r"(?:автоматическ\w*\s+(?:рассылк\w*|ответ\w*|сообщен\w*)"
            r"|(?:ответ|сообщен\w*).{0,24}автоматическ\w*"
            r"|\bавтоответ\w*\b"
            r"|\b(?:я|это|ответил|ответила)\s+(?:бот|ии|нейросет\w*)\b"
            r"|(?:подключ\w*|ответ\w*|пиш\w*).{0,20}\b(?:бот|ии)\b"
            r"|\b(?:бот|ии)\b.{0,20}(?:подключ\w*|ответ\w*|пиш\w*)"
            r"|сгенерирован\w*.{0,24}(?:\bии\b|нейросет\w*))",
            low,
        )
    ):
        problems.append("automation_disclosure")
    ...
    last_client_text_raw = (_client_history_lines[-1] if _client_history_lines else "")
    last_client_text = last_client_text_raw.lower().replace("ё", "е")

    # MOP_CLIENT_ECHO_GUARD_V1: small/local models may occasionally mirror the
    # customer turn verbatim. Never let a customer's own sentence become the
    # manager reply. Catch exact and near-contained normalized echoes.
    def _echo_norm(value: str) -> str:
        return " ".join(_policy_re.findall(r"[а-яёa-z0-9]+", str(value or "").lower().replace("ё", "е"), _policy_re.I))
    _reply_norm = _echo_norm(reply)
    _client_norm = _echo_norm(last_client_text_raw)
    if _reply_norm and _client_norm and len(_client_norm) >= 12:
        _shorter = min(len(_reply_norm), len(_client_norm))
        _longer = max(len(_reply_norm), len(_client_norm))
        if (
            _reply_norm == _client_norm
            or (
                _shorter >= 20
                and (_reply_norm in _client_norm or _client_norm in _reply_norm)
                and (_shorter / max(1, _longer)) >= 0.85
            )
        ):
            problems.append("client_echo")

    # MOP_NO_DUPLICATE_REPLY_GUARD_V1:
    # Never emit the same manager text twice in a row. This catches robotic
    # loops even when the repeated sentence is an imperative without "?".
    _ours_history = _history_role_messages(history, "ours")
    if _ours_history:
        _previous_reply_norm = _echo_norm(_ours_history[-1])
        if (
            _reply_norm
            and _previous_reply_norm
            and len(_reply_norm) >= 12
            and _reply_norm == _previous_reply_norm
        ):
            problems.append("duplicate_previous_reply")

    has_client_phone = _mop_phone_known("\n".join(_client_history_lines))
    asks_phone = bool(
        _policy_re.search(
    ...
    # TYPO_TOLERANT_MEASUREMENT_INTENT_V1:
    # A customer may write «замерик», «замершик», «не знаю размеры» etc.
    # Once the latest client turn means "I need a measurer / dimensions unknown",
    # asking dimensions again is a logical contradiction, not merely a typo.
    _asks_dimensions = bool(
        "?" in reply
        and _policy_re.search(r"(?:размер|площад|габарит)", low)
    )
    if _asks_dimensions and _mop_measurement_intent(last_client_text):
        problems.append("measurement_required_no_dimensions")

    # UNIVERSAL_ANSWERED_QUESTION_GUARD_V2: not every business question has a
    # hard-coded slot. Catch same/near-same questions generically after a real
    # client response, including short answers such as «нет» and «не знаю».
    if "?" in reply and _mop_reply_repeats_answered_question(reply, history):
        problems.append("repeated_answered_question:any")

    # MOP_DIALOG_CONTINUITY_GUARD_V1:
    # If we already asked a concrete qualification slot and the client answered
    # after that question, do not ask the same slot again. This is intentionally
    # slot-based rather than model-based, so it also works during provider drift.
    if "?" in reply:
        _blocks = _history_role_blocks(history)
        _slot_patterns = (
            ("phone", r"(?:номер(?:\s+телефона)?|телефон)"),
            ("location", r"(?:\bрайон\w*|\bметро\b|\bгород\w*)"),
            ("budget", r"(?:бюджет|сумм[а-я]*)"),
            ("dimensions", r"(?:размер|площад|габарит)"),
            ("market", r"(?:новострой|вторич)"),
            ("financing", r"(?:наличн|ипотек)"),
            ("rooms", r"(?:сколько\s+комнат|комнатн)"),
            ("timing", r"(?:к\s+какому\s+сроку|на\s+какой\s+срок|когда\s+нуж)"),
            ("address", r"(?:адрес|доставк)"),
            ("rent_buy", r"(?:аренд[а-я]*\s+или\s+покуп|покуп[а-я]*\s+или\s+аренд)"),
        )
        for _slot, _pattern in _slot_patterns:
            if not _policy_re.search(_pattern, low):
                continue
            _last_asked = -1
            for _idx, _block in enumerate(_blocks):
                if _block.get("role") != "ours":
                    continue
                _btxt = str(_block.get("text") or "").lower().replace("ё", "е")
                if "?" in _btxt and _policy_re.search(_pattern, _btxt):
                    _last_asked = _idx
            if _last_asked < 0:
                continue
            _answered_after = any(
                b.get("role") == "client" and str(b.get("text") or "").strip()
                for b in _blocks[_last_asked + 1:]
            )
            if _answered_after:
                problems.append("repeated_answered_question:" + _slot)
                break

    # A generated phrase must not claim that a handoff is already completed or
    # guarantee that a specialist will contact the client before BORIS has
    # actually created/assigned that human action in the business state.
    _completed_handoff = bool(
        _policy_re.search(
            r"\b(?:переда(?:л(?:и)?|ю|ем|ём|ете)|уже\s+передан\w*)\b.{0,110}"
            r"\b(?:специалист|менеджер|коллег|контакт|запрос|критери|пожелан)",
            low,
        )
        or _policy_re.search(
            r"\b(?:специалист|менеджер|коллег[а-я]*)\b.{0,70}\b(?:свяжется|позвонит|напишет|пришл[её]т|подготовит)\b",
            low,
        )
    )
    if _completed_handoff:
        problems.append("unconfirmed_completed_handoff")
...
def generate_ai_draft_reply(account_id: str, chat: dict, proactive_event: str = None,
                            ...
                            idempotency_key: str = None, ai_timeout_sec: int = None):
    ...
    try:
        ...
        # MOP_FIRST_TURN_ALL_ENTRYPOINTS_V1:
        # Proactive Avito system events are still the first company turn when
        # there is no earlier outgoing message. Greeting/company identity must
        # not disappear merely because the entrypoint is proactive.
        _first_manager_turn = bool(
            _mop_first_manager_turn(db, account_id, str(chat_id or ""), chat)
        )
        _first_account_intro = ""
        _first_account_name = str(getattr(account, "name", "") or "").strip()
        if _first_manager_turn:
            try:
                from app.api.client_memory import _first_contact_sales_reply as _first_contact_reply
                _intro = _first_contact_reply(
                    db, account_id, "", force=True, intro_only=True
                ) or {}
                _first_account_intro = str(_intro.get("answer") or "").strip()
            except Exception as _intro_exc:
                print(
                    "MOP_FIRST_TURN_INTRO_DEFERRED %s: %s"
                    % (account_id, str(_intro_exc)[:160]),
                    flush=True,
                )
                _first_account_intro = ""
    finally:
    ...
    def _finalize(text_value, usage_value, return_meta_value, analysis=None):
        """Apply account-specific greeting/introduction exactly once on first turn."""
        import re as _finish_re
        final_text = str(text_value or "").strip()
        if (
            _first_manager_turn
            and _first_account_intro
            and final_text
            and not final_text.startswith("[Ошибка")
        ):
            _greeting_re = _finish_re.compile(
                r"^\s*(?:здравствуйте|добрый\s+(?:день|вечер)|доброе\s+утро|привет)\b[!,.—:\s-]*",
                _finish_re.I,
            )
            _norm = lambda value: " ".join(
                _finish_re.findall(
                    r"[а-яёa-z0-9]+",
                    str(value or "").lower().replace("ё", "е"),
                    _finish_re.I,
            ...
            _intro_norm = _norm(_first_account_intro)
            _final_norm = _norm(final_text)
            if not (_intro_norm and _final_norm.startswith(_intro_norm)):
                # A model may output only a generic «Здравствуйте!» or repeat
                # the company name without the configured niche. Replace that
                # leading self-introduction with canonical account truth, while
                # preserving the substantive answer that follows.
                body = _greeting_re.sub("", final_text, count=1).strip()
                if _first_account_name and body:
                    _first_sentence = _finish_re.match(r"^([^.!?]+[.!?])(?:\s+|$)(.*)$", body, _finish_re.S)
                    if (
                        _first_sentence
                        and _norm(_first_account_name) in _norm(_first_sentence.group(1))
                    ):
                        body = str(_first_sentence.group(2) or "").strip()
                final_text = (
                    _first_account_intro.rstrip()
                    + ((" " + body) if body else "")
                ).strip()
        return _wrap(final_text, usage_value, return_meta_value, analysis)
    ...
    # MOP_VIEWED_PHONE_SAFE_CONTINUATION_V1:
    # Avito's «client viewed your number» is evidence that the client already
    # has the COMPANY contact. Never spend AI on this technical nudge and never
    # respond by asking the client to leave their own phone number.
    if proactive_event == "viewed_phone":
        _viewed_phone_reply = (
            "Если удобнее, можем продолжить здесь в чате — "
            "напишите, что хотите уточнить."
        )
        _viewed_phone_analysis = {
            "intent": "viewed_company_phone",
            "qualification_stage": "INQUIRY",
            "lead_temperature": "warm",
            "phone_received": False,
            "crm_action": "keep_inquiry",
            "target_action": "продолжить диалог в удобном клиенту канале",
            "next_action": "ответить на содержательный вопрос клиента",
        ...
        _viewed_phone_usage = {
            ...
            "model": "viewed_phone_safe_continuation_v1",
            "prompt_tokens": 0,
            "completion_tokens": 0,
            "total_tokens": 0,
            "cost_rub": 0.0,
            "memory_fast_path": True,
            "policy_version": "MOP_VIEWED_PHONE_SAFE_CONTINUATION_V1",
        }
        return _finalize(
            _viewed_phone_reply,
            _viewed_phone_usage,
            return_meta,
            analysis=_viewed_phone_analysis,
        )
    ...
    if (
        _question
        and _mop_phone_known(_question)
        ...
        return _finalize(_phone_reply, _phone_usage, return_meta, analysis=_phone_analysis)
    ...
    if _question:
        ...
        if _mem.get("use_memory") and _mem.get("mode") != "hybrid":
            ...
            if _mem.get("found"):
                ...
                return _finalize(_memory_reply(_mem), dict(_MEM_USAGE), return_meta)
            print("[memory] знания нет -> уточняющий ответ")
            return _finalize(str(_mem.get("reply") or "Уточню и вернусь к вам."), dict(_MEM_USAGE), return_meta)
        ...
        if (_mem.get("use_memory") and _mem.get("mode") == "hybrid"
            ...
            return _finalize(_memory_reply(_mem), _mu, return_meta)
        ...
        if (_mem.get("use_memory") and _mem.get("mode") == "hybrid"
            ...
            return _finalize(_reply, _mu, return_meta, analysis=_analysis)
        ...
        if (_mem.get("use_memory") and _mem.get("mode") == "hybrid"
            ...
            return _finalize(str(_mem.get("reply") or "Передам ваш вопрос менеджеру."), _mu, return_meta, analysis=_analysis)
    ...
    # META_NONANSWER_CONTINUITY_V1:
    # "I already wrote it / see above" is not an answer to the manager's last
    # qualification question. Re-ask that unresolved point instead of silently
    # advancing the sales cycle.
    if (
        _mop_client_meta_nonanswer(_question)
        and _history_role_messages(history_text, "ours")
    ):
        _meta_reply = _mop_meta_nonanswer_reply(history_text)
        _meta_analysis = _deterministic_mop_qualification(
            history_text,
            _meta_reply,
            {
                "human_handoff": False,
                "handoff_reason": None,
                "next_action": "получить ответ на последний незакрытый вопрос",
            },
        )
        _meta_usage = {
            "provider": "deterministic_guard",
        ...
        return _finalize(_meta_reply, _meta_usage, return_meta, analysis=_meta_analysis)
    ...
    if _question:
        ...
        try:
            ...
            _early_learning_rules = _mop_effective_learning_rules(
                _early_rules_for_mop(
                    _lrdb,
                    account_id,
                    shared=True,
                    only_confirmed=True,
    ...
    if _initial_subject_reply:
        ...
        return _finalize(_initial_subject_reply, _initial_usage, return_meta, analysis=_initial_analysis)

    _high_confidence_reply = (
        None
        if _learning_relevant_now
        else _mop_high_confidence_first_turn_reply(_question, history_text)
    )
    if _high_confidence_reply:
        _fast_analysis = _deterministic_mop_qualification(
            history_text,
            _high_confidence_reply,
            {"human_handoff": False, "handoff_reason": None,
             "next_action": "продолжить квалификацию с одного полезного параметра"},
        )
        _fast_usage = {
            "provider":"deterministic_guard", "model":"deterministic_guard",
            "prompt_tokens":0, "completion_tokens":0, "total_tokens":0,
            "cost_rub":0.0, "memory_fast_path":True,
            "policy_version":"MOP_HIGH_CONFIDENCE_FIRST_TURN_V1",
        }
        print("MOP_HIGH_CONFIDENCE_FIRST_TURN %s/%s" % (account_id, chat_id), flush=True)
        return _finalize(_high_confidence_reply, _fast_usage, return_meta, analysis=_fast_analysis)
    ...
    if _fact_guard:
        try:
            from app.services.mop_dialogue_brain import record_knowledge_gap
            record_knowledge_gap(
                account_id,
                chat_id,
                _question,
                str(_fact_guard.get("handoff_reason") or "unknown_fact"),
            )
        except Exception as _gap_exc:
            print("MOP_KNOWLEDGE_GAP_NOTIFY_FAIL %s/%s: %s" % (
                account_id, chat_id, str(_gap_exc)[:160]
            ), flush=True)
        ...
        return _finalize(_fact_guard["reply_text"], _guard_usage, return_meta, _guard_analysis)
    ...
    try:
        from app.services.mop_dialogue_brain import dialogue_context as _dialogue_context
        _dialogue_brain_block = _dialogue_context(account_id, max_chars=4200)
    except Exception:
        _dialogue_brain_block = ""
    if _dialogue_brain_block:
        goal_block += "\n\n" + _dialogue_brain_block + "\n"

    learning_rules = _mop_effective_learning_rules(list(_early_learning_rules))
    if not _early_learning_loaded:
        _rdb = SessionLocal()
        try:
            from app.api.client_memory import rules_for as _rules_for_mop
            learning_rules = _mop_effective_learning_rules(
                _rules_for_mop(
                    _rdb,
                    account_id,
                    shared=True,
    ...
    # MOP_LOCAL_COMPACT_PROMPT_V2:
    # Local Qwen is the zero-cost recovery layer. Keep its prompt compact enough
    # for CPU inference while preserving the same confirmed facts, owner training
    # and deterministic safety guards applied after generation.
    # MOP_LOCAL_COMPACT_PROMPT_V3: local qwen is a recovery layer, not a
    # duplicate of the full remote prompt. Keep only the last conversation,
    # concise confirmed facts/goal and effective owner training. Deterministic
    # output guards still validate the generated text afterwards.
    _local_prompt_text = (
        "Ты менеджер продаж компании в чате Avito. Ответь клиенту от лица компании.\n"
        + ("ФАКТЫ:\n" + company_context[:250] + "\n" if company_context else "")
        + (confirmed_memory_context[:350] + "\n" if confirmed_memory_context else "")
        + (item_context[:80] + "\n" if item_context else "")
        + (custom_prompt_block[:120] + "\n" if custom_prompt_block else "")
        + "ДИАЛОГ:\n" + history_text[-600:] + "\n"
        + goal_block[:180]
        + "\nПРАВИЛА:\n"
        "- Ответь именно на последнее сообщение. Максимум один вопрос.\n"
        "- Не выдумывай цену, срок, наличие, документы, скидки и обещания.\n"
        "- Не повторяй уже известный вопрос; если телефон есть, больше не проси.\n"
        "- Не утверждай, что что-то передал/сделал, если этого нет в истории.\n"
        "- Пиши по-русски живо, 1–2 коротких предложения, без рекламной воды.\n"
        "- Не называй себя Борисом, ИИ, ботом или придуманным именем.\n"
        + learning_priority_block[:320]
        + "- Верни только готовую реплику клиенту обычным текстом. Без JSON и пояснений.\n"
    )
    ...
    try:
        ...
        if idempotency_key:
            ...
            if cached:
                ...
                if _cached_policy:
                    ...
                    if {"automation_disclosure", "internal_or_invented_identity"} & _cpv:
                        _cached_text = (
                            "Понимаю. Давайте разберём ваш вопрос по существу — "
                            "уже известные данные повторно спрашивать не буду."
                        )
                    elif "phone_already_received" in _cpv:
                        _cached_text = "Контакт уже есть, повторно номер не нужен. Продолжим по текущему вопросу."
                    elif "duplicate_previous_reply" in _cpv:
                        _cached_text = _mop_account_next_question(account_id, history_text)
                    elif "measurement_required_no_dimensions" in _cpv:
                        _cached_text = _mop_measurement_repair_reply(history_text)
                    elif "repeated_answered_question:dimensions" in _cpv and _mop_measurement_intent((_history_role_messages(history_text, "client") or [""])[-1]):
                        _cached_text = _mop_measurement_repair_reply(history_text)
                    elif any(x.startswith("repeated_answered_question:") for x in _cpv):
                        _cached_text = _mop_account_next_question(account_id, history_text)
                    elif "unconfirmed_completed_handoff" in _cpv:
                        _cached_text = "Зафиксировал ваш запрос и данные. Повторно ничего присылать не нужно."
                    elif "unconfirmed_callback_time" in _cpv:
                        _cached_text = "Контакт уже есть. Точное время созвона лучше согласовать отдельно."
                    ...
                    _continuity_only = all(
                        x in {"client_echo", "phone_already_received", "duplicate_previous_reply", "automation_disclosure", "internal_or_invented_identity", "repeated_handoff_promise", "measurement_required_no_dimensions"}
                        or x.startswith("repeated_answered_question:")
                        for x in _cpv
                    )
                    _cached_analysis.update({
                        "human_handoff": False if _continuity_only else True,
                        "handoff_reason": None if _continuity_only else "client_reply_policy_guard:" + ",".join(_cached_policy),
                    })
                    return _finalize(_cached_text, cached.get("usage"), return_meta, _cached_analysis)
                return _finalize(_cached_text, cached.get("usage"),
        ...
        _prompt_text = system_prompt + _style_block(style_hint)
        _local_router_prompt = _local_prompt_text + _style_block(style_hint)
        ...
        if _vision_urls:
            ...
            try:
                _sales_result = _sales_ai.generate_text(
                    ...
                    prompt=_prompt_text,
                    local_prompt=_local_router_prompt,
                    module="mop",
                    ...
                    timeout=max(
                        10,
                        int(ai_timeout_sec) if ai_timeout_sec is not None
                        else int(os.getenv("BORIS_MOP_AI_TIMEOUT_SEC", "120") or 120),
                    ),
                    expect_json=True,
                    local_format=None,
                    local_temperature=0.10,
                    local_num_ctx=max(768, min(1280, int(os.getenv("BORIS_MOP_LOCAL_NUM_CTX", "1280") or 1280))),
                    local_num_predict=max(40, min(72, int(os.getenv("BORIS_MOP_LOCAL_NUM_PREDICT", "56") or 56))),
                )
                ...
                _emergency_next = _mop_account_next_question(account_id, history_text)
                _learned_emergency = _mop_deterministic_learning_reply(
                    _question,
                    learning_rules,
                )
                _prev_ours_emergency = _history_role_messages(history_text, "ours")
                if (
                    _mop_measurement_intent(_question)
                    and _prev_ours_emergency
                    and any(
                        cue in _prev_ours_emergency[-1].lower().replace("ё", "е")
                        for cue in ("размер", "площад", "габарит")
                    )
                ):
                    _emergency_text = _mop_measurement_repair_reply(history_text)
                elif _learned_emergency:
                    ...
                    _client_history = _history_role_messages(history_text, "client")
                    _ours_history = _history_role_messages(history_text, "ours")
                    _last_emergency_client = _client_history[-1] if _client_history else ""
                    _last_low = _last_emergency_client.lower().replace("ё", "е")
                    _all_client_low = "\n".join(_client_history).lower().replace("ё", "е")
                    _phone_known = bool(
                        _mop_phone_known(_all_client_low)
                        or _emergency_re.search(r"(?:телефон|номер|контакт).{0,40}(?:уже\s+)?(?:есть|оставил|оставила|оставлял|оставляла|отправил|отправила|писал|писала|дал|дала)", _all_client_low)
                    )
                    if _emergency_re.search(r"(?:что\s+дальше|дальше\s+что|следующ(?:ий|его)\s+шаг|что\s+теперь)", _last_low):
                        _emergency_text = "Новые данные от вас сейчас не нужны. Следующий шаг — обработка уже собранного запроса менеджером."
                    elif _phone_known and _emergency_re.search(r"(?:телефон|номер|контакт)", _last_low):
                        _emergency_text = "Контакт уже учтён, повторно номер не нужен. Остальные параметры запроса тоже сохраняю без повторных вопросов."
                    elif _emergency_re.search(r"^(?:да\b|верно\b|точно\b|только\b)", _last_low.strip()):
                        _emergency_text = "Принял уточнение. Буду исходить из уже указанных параметров и не стану спрашивать их заново."
                    elif len(_emergency_re.findall(r"[а-яa-z0-9]+", _last_low, _emergency_re.I)) >= 4:
                        _emergency_text = "Принял последнее уточнение. Уже указанные параметры сохраняю и повторно их не запрашиваю."
                    else:
                        _emergency_text = "Можем продолжить здесь. Напишите, пожалуйста, что именно хотите уточнить — отвечу без выдуманных условий."
                    if _ours_history and _emergency_text.strip().lower() == _ours_history[-1].strip().lower():
                        _emergency_text = "Уточнение принято. Повторно собирать уже известные данные не буду."
                return _finalize(
                    _emergency_text,
                    {
        ...
        # MEASUREMENT_REPLY_CONTEXT_V1:
        # If the previous MOP question was about dimensions and the client now
        # says (even with a typo) that a measurer is needed / dimensions are
        # unknown, this turn has a deterministic business meaning. Do not let
        # provider drift skip acknowledgement or ask dimensions again.
        _prev_ours_for_measure = _history_role_messages(history_text, "ours")
        if (
            _mop_measurement_intent(_question)
            and _prev_ours_for_measure
            and any(
                cue in _prev_ours_for_measure[-1].lower().replace("ё", "е")
                for cue in ("размер", "площад", "габарит")
            )
        ):
            reply_text = _mop_measurement_repair_reply(history_text)
        ...
        if _policy_violations:
            ...
            _knowledge_gap_violations = sorted(
                x for x in _pv
                if x.startswith("unconfirmed_")
                and x not in {"unconfirmed_completed_handoff", "unconfirmed_callback_time"}
            )
            if _knowledge_gap_violations:
                try:
                    from app.services.mop_dialogue_brain import record_knowledge_gap
                    record_knowledge_gap(
                        account_id,
                        chat_id,
                        _question,
                        "output_guard:" + _knowledge_gap_violations[0],
                    )
                except Exception as _gap_exc:
                    print("MOP_OUTPUT_GAP_NOTIFY_FAIL %s/%s: %s" % (
                        account_id, chat_id, str(_gap_exc)[:160]
                    ), flush=True)
            if {"automation_disclosure", "internal_or_invented_identity"} & _pv:
                reply_text = (
                    "Понимаю. Давайте разберём ваш вопрос по существу — "
                    "уже известные данные повторно спрашивать не буду."
                )
            elif "client_echo" in _pv:
                reply_text = _mop_echo_repair_reply(history_text)
            elif "phone_already_received" in _pv:
                reply_text = "Контакт уже есть, повторно номер не нужен. Продолжим по текущему вопросу."
            elif "duplicate_previous_reply" in _pv:
                reply_text = _mop_account_next_question(account_id, history_text)
            elif "measurement_required_no_dimensions" in _pv:
                reply_text = _mop_measurement_repair_reply(history_text)
            elif "repeated_answered_question:dimensions" in _pv and _mop_measurement_intent((_history_role_messages(history_text, "client") or [""])[-1]):
                reply_text = _mop_measurement_repair_reply(history_text)
            elif any(x.startswith("repeated_answered_question:") for x in _pv):
                reply_text = _mop_account_next_question(account_id, history_text)
            elif "unconfirmed_completed_handoff" in _pv:
                reply_text = "Зафиксировал ваш запрос и данные. Повторно ничего присылать не нужно."
            elif "unconfirmed_callback_time" in _pv:
                reply_text = "Контакт уже есть. Точное время созвона лучше согласовать отдельно."
            ...
            _continuity_only = all(
                x in {"phone_already_received", "duplicate_previous_reply", "automation_disclosure", "internal_or_invented_identity", "repeated_handoff_promise", "measurement_required_no_dimensions"}
                or x.startswith("repeated_answered_question:")
                for x in _pv
            )
            analysis.update({
                "human_handoff": False if _continuity_only else True,
                "handoff_reason": None if _continuity_only else "client_reply_policy_guard:" + ",".join(_policy_violations),
            })
        ...
        return _finalize(reply_text, usage_meta, return_meta, analysis)
    except Exception as e:
        return _finalize(f"[Ошибка генерации черновика: {str(e)[:150]}]", None, return_meta)
    finally:
...
# MOP_LEGACY_WHITELIST_STALE_SAFE_V1:
# Historical whitelist rows predate explicit mode metadata. Treat only newly
# saved mode=manual rows as strict; legacy rows must not silently suppress new
# owner-owned ads forever.
def get_messenger_item_whitelist_config(account_id: str):
    from app.db.session import SessionLocal
    from app.models.storage import Storage
    db = SessionLocal()
    try:
        ...
        if not row:
            return {"item_ids": [], "mode": "none"}
        try:
            cfg = _json.loads(row.value) if row.value else {}
        except Exception:
            cfg = {}
        ids = [str(x) for x in (cfg.get("item_ids") or []) if str(x).strip()]
        mode = str(cfg.get("mode") or "legacy").strip().lower()
        if mode not in {"manual", "legacy", "legacy_auto"}:
            mode = "legacy"
        return {"item_ids": ids, "mode": mode}
    finally:
        db.close()


def get_messenger_item_whitelist(account_id: str):
    """Compatibility view: return only item ids; mode lives in config helper."""
    return get_messenger_item_whitelist_config(account_id).get("item_ids", [])
...
def _system_proactive_events(account_id, queue_at):
    """Fresh unanswered Avito system events allowed to wake proactive MOP.

    This is deliberately a separate queue from normal incoming dialogue:
    - only explicit allowlisted system event kinds are returned;
    - events older than the bounded freshness window are ignored;
    - if a human/BORIS real outgoing already followed the event, it is closed;
    - the normal system-message draft guard remains intact.
    """
    import os as _os
    import time as _time
    from app.db.session import SessionLocal as _SL
    ...
    try:
        fresh_hours = max(1, min(24, int(
            _os.environ.get("MOP_SYSTEM_PROACTIVE_MAX_AGE_HOURS", "6") or 6
        )))
    except Exception:
        fresh_hours = 6
    qts = int(queue_at.timestamp()) if hasattr(queue_at, "timestamp") else int(queue_at or 0)
    min_ts = max(qts, int(_time.time()) - fresh_hours * 3600)

    db = _SL()
    try:
        rows = db.execute(_t(
            "WITH latest_sys AS ("
            " SELECT DISTINCT ON (s.avito_chat_id)"
            "        s.avito_chat_id,s.avito_message_id,s.avito_created_at,s.text"
            "   FROM messenger_messages s"
            "  WHERE s.account_id=:a"
            "    AND lower(s.direction) LIKE 'in%'"
            "    AND coalesce(s.msg_type,'')='system'"
            "    AND s.avito_created_at>:min_ts"
            "  ORDER BY s.avito_chat_id,s.avito_created_at DESC,s.id DESC"
            ")"
            " SELECT l.avito_chat_id,l.avito_message_id,l.avito_created_at,l.text"
            "   FROM latest_sys l"
            "  WHERE NOT EXISTS ("
            "    SELECT 1 FROM messenger_messages o"
            "     WHERE o.account_id=:a"
            "       AND o.avito_chat_id=l.avito_chat_id"
            "       AND lower(o.direction) LIKE 'out%'"
            "       AND coalesce(o.msg_type,'')<>'system'"
            "       AND o.avito_created_at>l.avito_created_at"
            "  )"
        ), {"a": account_id, "min_ts": min_ts}).fetchall()
        out = {}
        for chat_id, message_id, created_at, text_value in rows:
            event = _system_proactive_event(text_value)
            if not event or not chat_id or not message_id:
                continue
            out[str(chat_id)] = {
                "event": event,
        ...
        print("MOP_SYSTEM_PROACTIVE_QUEUE_ERR %s: %s" %
              (account_id, repr(exc)[:180]), flush=True)
        return {}
    finally:
        db.close()
...
def _mop_incoming(account_id, chat_id, avito_message_id, incoming_text, chat,
    ...
    try:
        ...
        # MOP_JOB_SEEKER_NO_SALES_V1:
        # Ordinary sales MOP accounts must not treat applicants as customer
        # leads or ask them for a phone number. Recruitment accounts/listings
        # remain exempt. A phone-only acknowledgement inherits the immediately
        # preceding job-seeker context so the second turn cannot re-enter sales.
        try:
            from app.services.kpi_lead_quality import (
                classify_job_seeker_text as _classify_job_seeker,
                _account_is_recruitment as _account_is_recruitment,
            )
            _item_ctx = (chat.get("context") or {}) if isinstance(chat, dict) else {}
            _item_value = (_item_ctx.get("value") or {}) if isinstance(_item_ctx, dict) else {}
            _job_item_title = str(_item_value.get("title") or "")
            _is_job_seeker = False
            _job_reason = ""
            if not _account_is_recruitment(db, str(account_id)):
                _is_job_seeker, _job_reason = _classify_job_seeker(
                    incoming_text or "", _job_item_title
                )
                if not _is_job_seeker and _mop_phone_known(incoming_text or ""):
                    _prev_job_rows = db.execute(_mop_sql("""
                        SELECT text
                          FROM messenger_messages
                         WHERE account_id=:a
                           AND avito_chat_id=:c
                           ...
                           AND coalesce(msg_type,'') <> 'system'
                           AND avito_message_id <> :m
                           AND btrim(coalesce(text,'')) <> ''
                         ORDER BY avito_created_at DESC,id DESC
                         LIMIT 4
                    """), {
                        "a": account_id,
                        "c": str(chat_id),
                        "m": str(avito_message_id),
                    }).fetchall()
                    _prev_job_text = " ".join(
                        str(row[0] or "") for row in reversed(_prev_job_rows)
                    )
                    _prev_is_job, _prev_reason = _classify_job_seeker(
                        _prev_job_text, _job_item_title
                    )
                    if _prev_is_job:
                        _is_job_seeker = True
                        _job_reason = "context_" + str(_prev_reason or "job_seeker")
            if _is_job_seeker:
                _job_st = _mc.begin_incoming(
                    db, account_id, str(chat_id), str(avito_message_id),
                    incoming_text or "", ai_enabled=False
                )
                _job_status = str(_job_st.get("status") or "")
                if _job_status == "draft_ready":
                    _closed, _prev = _mc.set_status(
                        db, _job_st["draft_id"], "no_reply_required",
                        ("draft_ready",), "closed_no_reply",
                        channel="web", actor_type="system",
                        actor_id="job_seeker_guard",
                        payload="job seeker excluded from sales MOP",
                        meta={
                            "reason": _job_reason or "job_seeker",
                            "policy_version": "MOP_JOB_SEEKER_NO_SALES_V1",
                            "external_action": False,
                        },
                    )
                    if _closed is not None:
                        _job_status = "no_reply_required"
                # MOP_JOB_SEEKER_IMMEDIATE_CRM_RECONCILE_V1:
                # Close any already-existing sales deal in the same inbound
                # cycle. The CRM bridge independently blocks future recreation;
                # System Brain remains the bounded recovery fallback.
                if _job_status == "no_reply_required":
                    try:
                        from app.services.brain_recovery import (
                            _safe_mop_job_seeker_crm_exclusion as _job_crm_reconcile,
                        )
                        _job_crm_result = _job_crm_reconcile(db) or {}
                        if not bool(_job_crm_result.get("passed")):
                            print(
                                "MOP_JOB_SEEKER_CRM_RECONCILE_DEGRADED %s/%s" %
                                (account_id, chat_id),
                                flush=True,
                            )
                    except Exception as _job_crm_exc:
                        # Never turn an applicant into a sales reply because CRM
                        # reconciliation failed. The periodic brain recovery will
                        # retry the same exact evidence without external action.
                        print(
                            "MOP_JOB_SEEKER_CRM_RECONCILE_ERR %s/%s: %s" %
                            (account_id, chat_id, repr(_job_crm_exc)[:180]),
                            flush=True,
                        )
                print(
                    "MOP_JOB_SEEKER_SKIP %s/%s message=%s reason=%s status=%s" %
                    (account_id, chat_id, avito_message_id, _job_reason, _job_status),
                    flush=True,
                )
                return _job_status or "no_reply_required"
        except Exception as _job_guard_exc:
            # Classifier failure must not block legitimate sales traffic.
            print(
                "MOP_JOB_SEEKER_GUARD_ERR %s/%s: %s" %
                (account_id, chat_id, repr(_job_guard_exc)[:180]),
                flush=True,
            )

        # MOP_STALE_INCOMING_NO_AUTOSEND_V1:
        # A durable queue may recover after a provider outage or a repaired
        # whitelist. Never send a multi-day-old answer as if it were fresh.
        # Old business inquiries become internal human handoffs; no external
        # send happens here. Default 24h keeps ordinary overnight recovery safe.
        if not proactive_event:
            try:
                import os as _stale_os
                ...
                _max_age = max(
                    ...
                    min(
                        259200,
                        int(_stale_os.environ.get(
                            "MOP_MAX_AUTOREPLY_AGE_SECONDS", "86400"
                        ) or 86400),
                    ),
                )
                _msg_epoch = db.execute(_mop_sql("""
                    SELECT avito_created_at
                      FROM messenger_messages
                     WHERE account_id=:a AND avito_message_id=:m
                ...
                _incoming_age = (
                    max(0.0, _stale_time.time() - float(_msg_epoch))
                    if _msg_epoch else 0.0
                )
                if _msg_epoch and _incoming_age > float(_max_age):
                    _stale_st = _mc.begin_incoming(
                        db, account_id, str(chat_id), str(avito_message_id),
                        incoming_text or "", ai_enabled=False
                    )
                    _stale_status = str(_stale_st.get("status") or "")
                    if _stale_status == "draft_ready":
                        _stale_summary = {
                            "human_handoff": True,
                            "handoff_reason": "stale_unanswered_client_inquiry",
                            "next_action": (
                                "Проверить актуальность запроса и ответить клиенту "
                                "по текущим условиям вручную."
                            ),
                            "quality_note": (
                                "Старое входящее не отправляется автоматически "
                                "после восстановления очереди."
                            ),
                        }
                        _held, _prev = _mc.set_status(
                            db, _stale_st["draft_id"], "human_required",
                            ("draft_ready",), "handed_to_human",
                            channel="web", actor_type="system",
                            actor_id="stale_incoming_guard",
                            payload="stale unanswered inbound recovered without autosend",
                            meta={
                                "reason": "stale_unanswered_client_inquiry",
                                "policy_version": "MOP_STALE_INCOMING_NO_AUTOSEND_V1",
                                "age_seconds": int(_incoming_age),
                                "max_age_seconds": int(_max_age),
                                "external_action": False,
                            },
                            extra_sql=", ai_summary=:summary",
                            extra_params={
                                "summary": _stale_json.dumps(
                                    _stale_summary, ensure_ascii=False
                                )
                            },
                        )
                        if _held is not None:
                            _stale_status = "human_required"
                            try:
                                from app.services.lead_notifications import queue_mop_events
                                queue_mop_events(
                                    db, int(_stale_st["draft_id"]), _stale_summary
                                )
                            except Exception as _notify_exc:
                                print(
                                    "LEAD_NOTIFICATION_QUEUE_STALE_ERROR %s: %s" %
                                    (_stale_st["draft_id"], type(_notify_exc).__name__),
                                    flush=True,
                                )
                            try:
                                _mc.push_card(db, _stale_st["draft_id"])
                            except Exception:
                                pass
                    print(
                        "MOP_STALE_INCOMING_HOLD %s/%s message=%s age=%s status=%s" %
                        (
                            account_id, chat_id, avito_message_id,
                            int(_incoming_age), _stale_status,
                        ),
                        flush=True,
                    )
                    return _stale_status or "human_required"
            except Exception as _stale_guard_exc:
                print(
                    "MOP_STALE_INCOMING_GUARD_ERR %s/%s: %s" %
                    (account_id, chat_id, repr(_stale_guard_exc)[:180]),
                    flush=True,
                )
...
def _process_account_messenger_check(account_id: str, telegram_chat_id: str):
    ...
    _whitelist_cfg = get_messenger_item_whitelist_config(account_id)
    whitelist = list(_whitelist_cfg.get("item_ids") or [])
    _strict_whitelist = str(_whitelist_cfg.get("mode") or "") == "manual"
    ...
    try:
        if _queue_at is None:
            ...
            # MOP_SYSTEM_PROACTIVE_NUDGE_ROUTER_V1:
            # Normal durable queue intentionally ignores msg_type=system. Add a
            # second, tightly allowlisted queue for fresh unanswered proactive
            # Avito events, otherwise empty-chat/phone-view events are invisible
            # to contour=new even though their behavior exists below.
            _system_proactive = _system_proactive_events(account_id, _queue_at)
            if _system_proactive:
                _known_ids = {str(c.get("id")) for c in _all if c.get("id")}
                _missing_system = set(_system_proactive) - _known_ids
                if _missing_system:
                    _raw_system = fetch_chats(account_id, unread_only=False)
                    if _raw_system.get("status") == "ok":
                        for _sc in list(_raw_system.get("chats", []) or []):
                            if str(_sc.get("id")) in _missing_system:
                                _all.append(_sc)
                _wanted.update(_system_proactive)
        ...
        for chat in chats_result.get("chats", []):
            ...
            if whitelist and _strict_whitelist:
                # Explicit mode=manual remains a hard owner-controlled scope.
                if str(item_value.get("id", "")) not in whitelist:
                    continue
            else:
                # Legacy whitelist rows are advisory only. New owner-owned ads
                # must not be silently lost because a historical list is stale.
                # Foreign items and vacancy/recruitment listings stay blocked.
            ...
            # Fresh allowlisted Avito system proactive events use their own
            # synthetic exactly-once message id. The raw system text is never
            # passed as client dialogue, so _mop_incoming's system guard remains
            # fail-closed for every ordinary call site.
            _system_info = _system_proactive.get(str(chat_id)) if _contour == "new" else None
            if not _system_info:
                _raw_system_text = str(((last_message.get("content") or {}).get("text") or ""))
                # Keep the explicit empty-chat classifier visible in the scheduler:
                # it is a contract-tested high-value event, not a generic system nudge.
                _system_empty_chat = _is_system_empty_chat_nudge(_raw_system_text)
                _raw_system_event = "empty_chat" if _system_empty_chat else _system_proactive_event(_raw_system_text)
                if _raw_system_event and last_message.get("id"):
                    _system_info = {
                        "event": _raw_system_event,
                        "message_id": str(last_message.get("id")),
                    }
            if _system_info:
                _event = str(_system_info.get("event") or "")
                _source_mid = str(_system_info.get("message_id") or "")
                _event_mid = "system:%s:%s" % (_event, _source_mid)
                if _event and _source_mid:
                    already_processed = _get_last_processed_message_id(account_id, chat_id)
                    if already_processed == _event_mid:
                        continue
                    if _contour == "new":
                        _event_result = _mop_incoming(
                            account_id, chat_id, _event_mid, "", chat, _event
                        )
                        if _event_result in {
                            "card_sent",
                            "auto_sent",
                            ...
                            _set_last_processed_message_id(
                                account_id, chat_id, _event_mid
                            )
                        continue

            # Actually empty chat object (older Avito representation).
            if not last_message:
                already_processed = _get_last_processed_message_id(account_id, chat_id)
                if already_processed == "empty_chat_greeted":
                    continue  # уже поздоровались с этим пустым чатом раньше
                if _contour == "new":
                    _empty_result = _mop_incoming(
                        account_id, chat_id, "empty:%s" % chat_id,
                        "", chat, "empty_chat"
                    )
                    if _empty_result in {
                        "card_sent",
                        ...
                        _set_last_processed_message_id(
                            account_id, chat_id, "empty_chat_greeted"
                        )
                    continue
            ...
            proactive_event = (
                "viewed_phone"
                if _is_system_viewed_phone_nudge(last_text)
                else None
            )
...
def _messenger_poll_loop():
    ...
    poll_interval = max(10, int(os.environ.get("MESSENGER_POLL_INTERVAL_SECONDS", 10)))
    ...
    while True:
        ...
        try:
            ...
            try:
                ...
                # MOP_ACTIVE_BOUND_PRIORITY_V1:
                # Default pool size is 8 and the MOP-bound fleet currently fits
                # inside it. Submit MOP-bound accounts first so unrelated inbox
                # work cannot put a fresh sales lead behind a long account queue.
                # Package entitlement is still resolved below; inactive bindings
                # remain cheap fail-closed checks and no API cadence is increased.
                account_candidates.sort(
                    key=lambda row: (0 if row[0] in bound_mop else 1, row[0])
                )
            finally:
                db.close()
